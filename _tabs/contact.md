---
icon: fa fa-envelope-open
comments: true
order: 6
---

# ❤️ Donate｜Support My Work

Hi, I’m [**Harry**](/about/){:target="_blank"}.

Since 2018, I’ve been consistently writing and sharing content about **iOS development, RPA automation, and CI/CD**, along with life stories, product reviews, and travel experiences.

By the end of 2025, I’ve published [**120+ articles**, totaling **hundreds of thousands of words**](/archives/){:target="_blank"}.

In addition to writing, I actively develop and maintain [**several open-source projects**](https://github.com/ZhgChgLi){:target="_blank"}, including: 
- [**ZMarkupParser**](https://github.com/ZhgChgLi/ZMarkupParser){:target="_blank"} – A library for parsing HTML and converting it into NSAttributedString
- [**XCFolder**](https://github.com/ZhgChgLi/XCFolder){:target="_blank"} – A tool that helps migrate legacy Xcode project structures to a modern folder layout
- [**ZMediumToMarkdown**](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"} – A utility for downloading and backing up Medium articles
- [**ZReviewTender**](https://github.com/ZhgChgLi/ZReviewTender){:target="_blank"} – A service that monitors App Store reviews and forwards them to Slack in real time
- [**linkyee**](https://github.com/ZhgChgLi/linkyee){:target="_blank"} – A simple and free Linktree-like personal page

All of my work is created during evenings and weekends, and every project is **built with ❤️**.

If my articles or open-source tools have been helpful to you, **feel free to support me with a donation** and buy me a beer 🍺

Your support helps me stay motivated to keep writing, building, and contributing to the developer community.

**Thank you — it truly means a lot.**

<a href="https://www.paypal.com/ncp/payment/CMALMPT8UUTY2" target="_blank" rel="noopener" style="display:block;margin:12px 0;text-align:center;align-items:center;justify-content:center;padding:10px 22px;border-radius:999px;text-decoration:none;background:linear-gradient(135deg,#ffd54f,#ffb300);color:#000;font-size:16px;font-weight:700;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;box-shadow:0 4px 10px rgba(0,0,0,.15);cursor:pointer;transition:all .15s ease;width:350px" onmouseover='this.style.background="linear-gradient(135deg,#FFE082,#FFC107)",this.style.transform="translateY(-1px)",this.style.boxShadow="0 6px 14px rgba(0,0,0,0.2)"' onmouseout='this.style.background="linear-gradient(135deg,#FFD54F,#FFB300)",this.style.transform="translateY(0)",this.style.boxShadow="0 4px 10px rgba(0,0,0,0.15)"'>🍺 Buy me a beer on PayPal</a>
## Sponsors
### 2024/08 [**Common Room**](https://www.common-room.hk/c/boatp){:target="_blank"}
![](/assets/images/commonroom.png){: width="500" }

> Sponsored on: [My Bangkok Travelogue Post](/posts/b7e7c0938985/){:target="_blank"}
{: .prompt-tip }

## Supporters
### 2025
- 2025/12 [小克](https://buymeacoffee.com/goodjack){:target="_blank"} **🍺x10**: `支持分享～`
- 2025/12 [ethanhuang13](https://buymeacoffee.com/ethanhuang13){:target="_blank"} **🍺x5**: `請多多分享！`
- 2025/10 [shub](https://buymeacoffee.com/zhgchgli/c/16478672){:target="_blank"} **🍺x10**: `Thank you for the elegant solution! (linkyee)`
- 2025/10 [@dawei84_dev](https://x.com/dawei84_dev){:target="_blank"} **🍺x1**

### 2024
- 2024/09 [castropodcasts](https://buymeacoffee.com/zhgchgli/c/11031040){:target="_blank"} **🍺x5**

### 2023
- 2023/03 [@caocanx](https://x.com/caocanx){:target="_blank"} **🍺x5**

Last updated: 2026-01-02

---

### Email
- [zhgchgli@gmail.com](mailto:zhgchgli@gmail.com){:target="_blank"}

### Social
- [Linkedin](https://www.linkedin.com/in/zhgchgli/){:target="_blank"}
- [Twitter](https://twitter.com/zhgchgli){:target="_blank"}

...
description: 使用有限状态机与 Design Patterns 封装 DispatchSourceTimer，使其更安全易用。
image:
...
- simplified-chinese
title: iOS Timer 与 DispatchSourceTimer 如何选择与安全的使用?
---
...
- `.default` ：预设加入的 Mode，主要是处理 UI 显示。
  **会在切换到 `.tracking` Mode 时先暂停**
...
- ❌ suspend( ) 与 resume( ) 没有成对使用
  suspend( ) 后又呼叫一次 suspend( )
  resume( ) 后又呼叫一次 resume( )
...
感谢 [Ethan Huang](https://medium.com/u/e13f6afcf9b9){:target="_blank"} 大大 Donate 的 **5 Beers** :
...
- [**Design Patterns 的实战应用纪录 (封装 Sockiet.io)**](../78507a8de6a5/)


- [Design Patterns 的实战应用纪录 (封装 WKWebView)](../f4b02ee342a4/)

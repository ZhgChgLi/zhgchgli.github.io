...
另外一種方式是設定 JSONDecoder 的 keyDecodingStrategy，若 Response 資料欄位與 Property Name 僅為 `snake _ case` < \-** > `camelCase` 區別，可直接設定 \`.`** `keyDecodingStrategy` = \`.`** `convertFromSnakeCase` 就能自動匹配 Mapping。
```swift
let jsonDecoder = JSONDecoder()

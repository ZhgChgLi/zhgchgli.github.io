...
last_modified_at: 2022-04-07T15:06:40.478Z
...
![Photo by [Daniel McCullough](https://unsplash.com/@d_mccullough?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText){:target="_blank"}{:target="_blank"}](/assets/ba5773a7bfea/1*Q1BLU8QHVBLEMx6KlMSHWQ.jpeg)
...
![圖片取自 [refactoringguru](https://refactoringguru.cn/design-patterns/visitor){:target="_blank"}{:target="_blank"}](/assets/ba5773a7bfea/1*rbswlsges8_oS3pNI1-WKA.png)
...
![](/assets/ba5773a7bfea/1*ad2ijo5Bvm9_wnM1g2LNog.png){:target="_blank"}
...
![周星馳 — 食神](/assets/ba5773a7bfea/1*5kBPDRNpaHNyW4u4YEsOGA.png){:target="_blank"}
...
class ShareManager {
    ...
    func shareToFacebook() {
        ...
        print("[![\(self.title)](\(String(describing: self.imageURLStrings.first){:target="_blank"})](\(self.urlString){:target="_blank"})")
    }
...
class FacebookShare: ShareManagerProtocol {
    ...
    func share() {
        // call Facebook share sdk...
        print("Share to Facebook...")
        print("[![\(model.getShareText())](\(String(describing: model.getShareImageURLStrings(){:target="_blank"}.first))](\(model.getShareURLString(){:target="_blank"})")
    }
}
...
class ShareToFacebookVisitor: SharePolicy {
    func visit(model: UserModel) {
        // call Facebook share sdk...
        print("Share to Facebook...")
        print("[![Hi 跟你分享一位很讚的藝人\(model.name)。](\(model.profileImageURLString){:target="_blank"}](https://zhgchg.li/user/\(model.id){:target="_blank"}")
    }
    
    func visit(model: SongModel) {
        // call Facebook share sdk...
        print("Share to Facebook...")
        print("[![Hi 與你分享剛剛聽到一首很讚的歌，\(model.user.name) 的 \(model.name)，他被播方式。](\(model.coverImageURLString){:target="_blank"}{:target="_blank"})](https://zhgchg.li/user/\(model.user.id){:target="_blank"}/song/\(model.id)")
    }
...
_Converted [Medium Post](https://medium.com/zrealm-ios-dev/visitor-pattern-in-ios-swift-ba5773a7bfea) by [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown)._

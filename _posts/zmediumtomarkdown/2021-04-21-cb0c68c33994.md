...
last_modified_at: 2021-04-23T04:11:54.974Z
...
- APP \_** ID：前往 App 網頁版，會得到網址：https://apps \.** apple \.** com/tw/app/APP名稱/id **12345678** ，id 後面的數字及為 App ID（純數字）。
...
此指令會完成網頁登入驗證 \(** 包含兩步驟認證 \)** ，然後將 cookie 存入 FASTLANE \_** SESSION 檔案之中。
...
SpaceShip 裡面已經幫我們打包好撈評價列表的方法 [**Class: Spaceship::TunesClient::get \_** reviews**](https://www.rubydoc.info/gems/spaceship/0.39.0/Spaceship/TunesClient#get_reviews-instance_method){:target="_blank"} 了！
```
...
**首先** ，因原本的 get \_** reviews 口的參數不符合我們需求；我想要的是全地區、全版本的評價資料、不需要篩選、支援分頁：
```ruby
...
所以我們自己在 TunesClient 中擴充一個方法，裡面參數只帶 app \_** id、platform = **`ios`** \(** **全小寫** \)** 、index = 分頁 offset。
...
`SLACK _ WEB _ HOOK _ URL` = [**Incoming WebHook URL**](https://slack.com/apps/A0F7XDUAZ-incoming-webhooks){:target="_blank"}
### **最終結果**
```ruby
require "Spaceship"
require 'json'

...
Starting from iOS ≥ 16, when a user does not actively paste, if an app tries to read the clipboard, a prompt will appear asking for permission. The app will only be able to access clipboard information if the user allows it.
...
The clipboard may contain personal information, including passwords, such as those copied from password managers like 1Password or LastPass. If an app can read this information, it can also send it back to a server for logging. Ultimately, it depends on the developer's integrity. If someone wants to investigate, they can use [man-in-the-middle sniffing](../46410aaada00/) to monitor what data the app sends back to the server, including clipboard information.
### Background

The [Clipboard API](https://developer.apple.com/documentation/uikit/uipasteboard){:target="_blank"} has existed since iOS 3 in 2009, but starting with iOS 14, users are notified when their clipboard is accessed. Over the years, malicious apps could have already collected sufficient data.
### Why

Why do so many apps, both domestic and international, read the clipboard **upon opening**?

It's important to clarify that I'm referring to the situation when the **app is opened**, not when the app is actively using the clipboard. Reading the clipboard during app usage is typically related to specific functionalities, like Google Maps automatically pasting a recently copied address. However, some apps may continuously steal clipboard information.
...
The primary reason apps read the clipboard upon opening is to implement **[iOS Deferred Deep Link](../b08ef940c196/)** to **enhance user experience**, as shown in the flow above. When a product offers both a website and an app, we prefer users to install the app (due to higher engagement). Therefore, when users browse the website, we guide them to download the app, and we want the app to automatically open the page they were viewing when they left the website.


> _EX: When I browse the mobile version of PxHome on Safari -> see a product I like and want to buy -> PxHome wants to drive traffic to the app -> download the app -> open the app -> display the product I just saw on the website._ 
...
This is a **cat-and-mouse game** because Apple does not want developers to be able to reverse-track user sources. Before iOS 9, information was stored in web cookies, which apps could read after installation. After iOS 10, this method was blocked by Apple, leaving developers with no choice but to resort to the last option — "using the clipboard to transmit information." With iOS 14, Apple introduced a new prompt to make developers uncomfortable.

Another option is to use [Branch.io](https://branch.io/){:target="_blank"} to record user profiles (IP, device information) and then read the information through a matching process. This is theoretically feasible but requires significant manpower (involving backend, database, and app) to research and implement, and it may lead to misjudgments or collisions.


> _*Android Google already supports this functionality without the need for the roundabout methods required by iOS._ 
...
Many app developers may not even realize they have clipboard privacy issues because Google’s Firebase Dynamic Links service uses the same principle:
```javascript
...
A: Personally, I believe it's just an excuse to deflect public opinion. TikTok's intention seems to be "to prevent users from copying and pasting advertising messages everywhere"; however, it could effectively block and filter this when the message is completed or sent, without the need to constantly monitor the user's clipboard! Should the clipboard also be monitored for ads or "sensitive" information? I haven't posted anything.
...
Check if the clipboard content is a URL (since the content copied from the webpage JavaScript is a URL with parameters) before reading it, so that the clipboard access prompt does not appear every time the app is opened.
...
- **Have the app show a customized alert to inform users before reading the clipboard (to reassure users)**
- Add new privacy policy terms
- **Utilize the latest App Clips in iOS 14? Guide users from the webpage to lightweight App Clips for deeper interactions with the app.**

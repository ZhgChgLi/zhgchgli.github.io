...
> _In the previous article "[Crashlytics \+ Big Query Creating a More Immediate and Convenient Crash Tracking Tool](../e77b80cc6f89/)", we exported Crashlytics crash logs to Big Query and used Google Apps Script to schedule automatic queries for the Top 10 crashes and publish messages to a Slack channel._

This article continues by automating an important metric related to app crashes — **Crash\-Free Users Rate, the percentage of users unaffected by crashes**. Many app teams likely track and record this metric continuously, traditionally done through manual queries. The goal of this article is to automate this repetitive task and avoid potential errors when manually querying data. As mentioned before, Firebase Crashlytics does not provide any API for users to query, so we will similarly need to connect Firebase data to other Google services and then query the relevant data through that service's API.

![](/assets/793cb8f89b72/1*nvZXYgkj_8AdqHdR_yTCWg.png)

Initially, I thought this data could also be queried from Big Query; however, this approach was completely wrong because Big Query contains raw crash data and does not include data on users who did not crash, making it impossible to calculate the Crash\-Free Users Rate. There is not much information available online regarding this requirement, but after searching for a while, I found someone mentioning the keyword Google Analytics. I knew that Firebase's Analytics and Events could be connected to GA for queries, but I didn't expect that the Crash\-Free Users Rate would also be included. After reviewing the GA API, bingo!
...
Now that we know the way forward, we can start implementing it!
...
You can click on Extensions \-&gt; Apps Script in Google Sheets to create a Google Apps Script, or [click here for Google Apps Script](https://script.google.com/home/start){:target="_blank"} \-&gt; and click "New Project" in the top left corner.

![](/assets/793cb8f89b72/1*81_RPPZgBDvW4XplOHGmVg.png)

Once inside, you can click on the unnamed project name at the top to give your project a name.
...
Return to the earlier [GA4 Query Explorer](https://ga-dev-tools.web.app/ga4/query-explorer/){:target="_blank"} tool, and next to the "Make Request" button, you can check "Show Request JSON" to obtain the Request JSON for these conditions.
...
```
- GA Property ID: This can also be obtained from the earlier [GA4 Query Explorer](https://ga-dev-tools.web.app/ga4/query-explorer/){:target="_blank"} tool:
...
- googleSheetName: The name of the sheet in Google Sheets where crash records are kept.

![](/assets/793cb8f89b72/1*5lCtwwr3kZlBEEoW_D33gw.jpeg)

Paste the above code into the code block on the right in Google Apps Script, and select the "execute" function in the run method dropdown, then click Debug to test if it can retrieve data correctly:
...
Complete the account authorization steps as instructed.
...
Click the Run or Debug button again to execute "execute".

![](/assets/793cb8f89b72/1*tO7f0t5if6Db_eiv5BLOUQ.png)

Return to Google Sheets, and the data should have been successfully added!
#### Add Trigger to Schedule Automatic Execution

![](/assets/793cb8f89b72/1*MGO4FhC_8N8ul9dXZRYaMg.jpeg)

Select the clock button on the left \-&gt; then click "+ Add Trigger" at the bottom right.
...
From now on, the tracking of App Crash\-Free Users Rate data is fully automated; no manual querying or filling is required; everything is handled automatically by the machine!

> We only need to focus on solving app crash issues!

> _p.s. Unlike the previous article where querying data from Big Query incurs costs, this article's querying of Crash\-Free Users Rate using Google Apps Script is completely free, so you can use it with confidence._

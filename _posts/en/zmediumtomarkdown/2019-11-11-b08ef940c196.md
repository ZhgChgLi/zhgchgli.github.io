...
After graduating and serving in the military, I have been working for almost three years now. My growth has plateaued, and I've started to settle into a comfort zone. Fortunately, I made the bold decision to resign and take a step back to reset.

While reading [Designing Your Life](https://www.books.com.tw/products/0010733134){:target="_blank"}, I reflected on my career and life. Although my technical skills are not exceptional, writing on Medium and sharing with others allows me to enter a "flow state" and gain a lot of energy. Recently, a friend asked me about Deep Link issues, which prompted me to organize my research and also recharge my own energy!
### Scenarios
...
3\. Special event entries, for example, downloading and opening through a specific URL can earn rewards.
#### Support:
...
#### First, we need to add a prompt for "if not, redirect to the App Store" to inform users to install the app:

The **URL Scheme** part is controlled by the system and is rarely made public for internal app calls; if the trigger point is in an area that you cannot control (e.g., Line link), it cannot be handled.
...
The basic logic is to **call the URL Scheme, then set a timeout. If the page hasn't redirected after the timeout, it assumes the scheme cannot be called, and redirects to the App Store page** (though the experience is still poor and may show an error message; it just adds automatic redirection).

**Universal Link** is essentially its own webpage. If there is no redirection, it defaults to displaying in a web browser. If you have web services, you can choose to directly open the webpage; if not, it redirects to the App Store page.
...
![Flowchart for adding "if not, redirect to the App Store"](/assets/b08ef940c196/1*B-_5tIDWQpNO8NxpXQsEcA.jpeg)

Flowchart for adding "if not, redirect to the App Store"
#### Complete Deep Link Handling on the App Side:

What we want is not just "if the user has the app installed, then open the app," but also to connect the source information with the app, allowing the app to automatically display the target page after opening.
...
First, here’s a URL extension method `queryParameters` to conveniently convert URL queries into a Swift Dictionary.
```swift
...
What is a Deferred Deep Link? It is a delayed deep link; it allows our Deep Link to extend to retain source data even after the app is installed from the App Store.

According to Android engineers, Android already has this feature, but iOS does not support this setting, and achieving this functionality is not user-friendly. Please continue reading.
### Deferred Deep Link
...
One method calculates a hash value based on user device, IP, environment, etc., and stores data on the server; when the app is installed and opened, it calculates the value in the same way. If the values match, it retrieves the data (this is how branch.io works).

The other method is the one I will introduce in this article, which uses the iPhone clipboard and the sharing mechanism of Safari and app cookies, effectively storing data in the clipboard or cookies, and the app reads it after installation.

![](/assets/b08ef940c196/1*VVahSlHV2N2jcIw4afzr2g.jpeg)

```
After clicking "Open," your clipboard will be automatically overwritten with JavaScript to copy the relevant jump information: https://XXX.app.goo.gl/?link=https://XXX.net/topicID=1&type=topic
```

Those who have used Firebase Dynamic Links will be familiar with this opening jump page, and once you understand the principle, you will know that this page cannot be removed from the process!
...
If the app only considers iOS ≥ 10, it becomes much easier. The app can implement clipboard access, and the web can use JavaScript to overwrite information to the clipboard, then redirect to the App Store for download.
...
> _According to official documentation, starting from iOS 11, it is no longer possible to access the user's Safari cookies. If you need this functionality, you can use SFAuthenticationSession, but this method cannot run in the background and will prompt the following dialog each time it loads:_ 
...
Let's start with the simple part, considering only users on iOS ≥ 10, using the iPhone clipboard to transfer information.
#### Web Side:
...
Now comes the tricky part: supporting iOS 9. As mentioned earlier, since clipboard access is not supported, we need to use **Cookie sharing**.
#### Web Side:
...
**Another pitfall:** The secret loading of SFSafariViewController; if its size is set to less than 1, transparency less than 0.05, or set to isHidden, SFSafariViewController will **not load**.
...
My approach here is to place a UIView at the top of the main page's UIViewController with an arbitrary height, aligning its bottom with the top of the main page's UIView. Then, I connect the IBOutlet `sharedCookieView` to the class. In the `viewDidLoad()` method, I initialize the SFSafariViewController and add its view to `sharedCookieView`. So, it actually loads and displays, but it's off-screen, making it invisible to the user 🌝.

**What URL should the SFSafariViewController point to?**

Similar to the web sharing page, we need to create a separate page for reading cookies and place both pages under the same domain to avoid cross-domain cookie issues. I will attach the page content later.
```swift
...
Here, I couldn't find a method to read cookies from SFSafariViewController; the methods I found online all returned empty results.

> _Alternatively, you might need to use JavaScript to interact with the page content, instructing JavaScript to read the cookies and return them to the UIViewController._ 
...
If you find this tedious, you can directly use [branch\.io](http://branch.io){:target="_blank"} or [Firebase Dynamic](https://firebase.google.com/docs/dynamic-links){:target="_blank"} without reinventing the wheel. This is necessary here due to custom interface requirements and some complex needs, so I had to build it myself.

Users on iOS 9 are already quite rare, so if it's not essential, you can ignore it; using the clipboard method is quick and efficient, and it doesn't limit the link to being opened only in Safari!

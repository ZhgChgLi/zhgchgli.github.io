...
description: "Hands-on creation of an uninterrupted app redirection process suitable for all scenarios"
image:
...
Hands-on creation of an uninterrupted app redirection process suitable for all scenarios
...
After graduating and serving in the military, I have been working for almost three years. My growth has plateaued, and I've started to settle into a comfort zone. Fortunately, I made the bold decision to resign and take a step back to reset.

While reading [Designing Your Life](https://www.books.com.tw/products/0010733134){:target="_blank"}, I revisited my life plans. Although my technical skills are not exceptional, writing on Medium and sharing with others allows me to enter a "flow state" and gain a lot of energy. Recently, a friend asked me about Deep Link issues, which prompted me to organize my research and recharge my energy!
...
3. Special event entries, for example, downloading and opening via a specific URL to receive rewards.
#### Support:
...
#### First, we need to add a prompt for "if not installed, redirect to the App Store" to inform users to install the app:

The **URL Scheme** part is controlled by the system and is rarely made public for internal app calls; if the trigger point is in an area that cannot be controlled (e.g., Line link), it cannot be processed.
...
The basic logic is to **call the URL Scheme, then set a timeout. If the page hasn't redirected after the timeout, it assumes the scheme couldn't be called because the app isn't installed, and redirects to the App Store page** (though the experience is still poor and may show an error message, it at least adds automatic redirection).

**Universal Link** is essentially a webpage of its own. If there is no redirection, it defaults to displaying in a web browser. If you have web services, you can choose to directly open the webpage; if not, it redirects to the App Store page.
...
![Flowchart for adding "if not installed, redirect to App Store"](/assets/b08ef940c196/1*B-_5tIDWQpNO8NxpXQsEcA.jpeg)

Flowchart for adding "if not installed, redirect to App Store"
#### Complete Deep Link Handling on the App Side:

What we want is not just "if the user has the app installed, then open the app." We also want to connect the source information with the app, allowing the app to automatically display the target page after opening.
...
The **Universal Link** is handled in the AppDelegate with `func application(_ application: UIApplication, continue userActivity: NSUserActivity, restorationHandler: @escaping ([Any]?) -> Void) -> Bool`:
```swift
extension URL {
...
Here’s a URL extension method `queryParameters` for easily converting URL queries into a Swift Dictionary.
```swift
...
It seems we have covered all possible scenarios, so what else is missing?

![](/assets/b08ef940c196/1*ulrLKyvTKoChPScWD9wHyA.jpeg)

As shown in the diagram, if the user has not installed the app -> installs from the App Store -> opens from the App Store, the source data will be lost, and the app will not know the source, so it will only display the homepage; the user must go back to the previous webpage and click again to open the app for it to navigate to the desired page.
...
One method calculates a hash value based on user device, IP, environment, etc., and stores data on the server from the webpage; when the app is installed and opened, it calculates the value in the same way. If the values match, it retrieves the data (this is how branch.io works).

The other method, which this article will introduce, uses the iPhone clipboard and the shared mechanism of Safari and app cookies, effectively storing data in the clipboard or cookies, which the app can read after installation.

![](/assets/b08ef940c196/1*VVahSlHV2N2jcIw4afzr2g.jpeg)

```
After clicking "Open," your clipboard will be automatically overwritten with JavaScript, copying the relevant redirection information: https://XXX.app.goo.gl/?link=https://XXX.net/topicID=1&type=topic
```

Those who have used Firebase Dynamic Links will be familiar with this opening redirection page; once you understand the principle, you will realize that this page cannot be removed from the process!
...
Additionally, the app needs to secretly load SFSafariViewController in the background to retrieve the cookie information stored when the link was clicked.
...
> _According to official documentation, iOS 11 no longer allows access to the user's Safari cookies. If you need this functionality, you can use SFAuthenticationSession, but this method cannot run in the background and will always prompt the following window before loading:_
...
> _Furthermore, app review does not allow placing SFSafariViewController in areas not visible to the user. \(Triggering it programmatically and then adding it as a subview is not easily detectable\)_ 
...
The web side is relatively easy to handle; we will change it so that when the user clicks "Go Now," we store the information we want to send to the app in a cookie `（marry://topicID=1&type=topic）`, and then use `location.href` to redirect to the App Store page.
...
**Another pitfall:** The SFSafariViewController's view will **not load** if its size is set to less than 1, its opacity is less than 0.05, or it is set to isHidden.
...
My approach here is to place a UIView at the top of the main UIViewController with an arbitrary height, aligning its bottom with the top of the main UIView. Then, I connect the IBOutlet `sharedCookieView` to the class. In the `viewDidLoad()` method, I initialize the SFSafariViewController and add its view to `sharedCookieView`. So, it actually shows that it has loaded, but it runs off the screen, making it invisible to the user 🌝.

**What URL should the SFSafariViewController point to?**

Similar to the web sharing page, we need to create a page for reading cookies and place both pages under the same domain to avoid cross-domain cookie issues. The page content will be attached later.
```swift
...
At this point, you might think that the next step is to read the cookies from the webpage in `didCompleteInitialLoad`, and then you're done!
...
Since iOS doesn't know how to retrieve shared cookies, we can let the "cookie reading page" help us "read the cookies."

The `getCookie()` method from the JavaScript cookie handling mentioned earlier is used for this. Our "cookie reading page" is just a blank page (since the user won't see it), but in the JavaScript part, we need to read the cookies after the body onload:
```xml
<html>
<head>
...
Thus, the corresponding callback handling will return to the `AppDelegate` in `func application(_ application: UIApplication, open url: URL, sourceApplication: String?, annotation: Any) -> Bool` for processing.
### Done! Summary:

![](/assets/b08ef940c196/1*kp26TdlJBW5sVxw4zYa9Rg.jpeg)

If you find this tedious, you can directly use [branch\.io](http://branch.io){:target="_blank"} or [Firebase Dynamic](https://firebase.google.com/docs/dynamic-links){:target="_blank"}—there's no need to reinvent the wheel. This is due to the need for interface customization and some complex requirements, which is why I had to build it myself.

Users on iOS 9 are already quite rare, so if it's not necessary, you can ignore it; using the clipboard method is quick and efficient, and it doesn't limit the link to be opened only in Safari!

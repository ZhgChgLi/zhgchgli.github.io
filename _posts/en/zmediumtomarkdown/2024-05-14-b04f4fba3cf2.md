...
After working for a long time and handling many products, I have started to encounter products I once participated in that are about to reach their final destination (removal); developing a product from scratch is like nurturing a new life. The team worked hard for 3–4 months to bring this "child" into the world; although it has since been handed over to other caregivers (engineers) for further development, hearing that it is about to enter the end of its product life cycle still brings a sense of regret.

> **_Life is like this; we never know whether tomorrow will bring the sunrise or an unexpected event. The only thing we can do is cherish the present and do our best._**

#### Remembrance

Every journey leaves a trace. We hope to do something before the product reaches its final destination, allowing everyone to have a chance to reminisce and at least leave proof of its existence. The following methods require the App to still be online; if it has already been removed, only memories remain.

### Non-Technical Method — Recording

In addition to using the [built-in screen recording feature on the iPhone](https://support.apple.com/zh-tw/102653){:target="_blank"}, we can also use QuickTime Player to connect the phone to a Mac for recording and exporting videos.
...
3. When the recording interface pops up, click the "v" next to the red button, and select your connected phone for the screen and audio.
...
The benefit of video memories is that they connect future recollections more easily than images. The deeper the recording, the more detailed the documentation. If you want to convert specific frames into images, you can also take screenshots directly, which is very convenient.
...
- The skeleton will disappear once the App is removed from the App Store.
- The flesh will disappear when the API host or server is shut down.

Thus, we also separate the backup of the skeleton and flesh into two technical methods.
...
Once the App is removed from the store, it will remain on any phone that has downloaded it as long as the App is not actively deleted. If you switch phones, it will also transfer over.

However, if we accidentally delete the App or do not transfer it when changing phones, we will truly say goodbye forever; at this point, if we have manually backed up the .ipa file from the store, we can revive it.

A long time ago, an article on [reverse engineering](../7498e1ff93ce/) mentioned this, but this time we only need to back up the .ipa file without cracking it, using only the tools provided by Apple.
...
A waiting window will appear, indicating that the App is being added to XXX and downloading "XXX".
...
> **_At this point, do not press any buttons. Do not press any buttons. Do not press any buttons._**
...
Similarly, connect the phone where you want to restore the App to your Mac and open Apple Configurator 2 to enter the App addition interface.
...
Here, we will use the method from the previous article [**App End-to-End Testing Local Snapshot API Mock Server**](../5a5c4b25a83d/) (details can be referenced there) along with the open-source project created at that time:
...
Using the same technique as recording API Request & Response for E2E Testing, we can also use it to document the last API Request & Response Data before the App is removed or goes offline.
...
If you are using it purely for network request sniffing and are not accustomed to the mimproxy interface, you can also switch to using [Proxyman](https://proxyman.io/){:target="_blank"}; you can refer to my previous [another article](../70a1409b149a/) for usage.

**2. Complete mitmproxy certificate setup**

> _For HTTPS encrypted connections, we need to use a root certificate for the man-in-the-middle attack, so the first time you use it, you must download and enable the root certificate on your mobile device._
...
Seeing this screen means the mitmproxy service has started, and currently, there is no traffic coming in, so it is empty. Do not close the Terminal while on this screen.
- Check the Mac's network settings to find the Mac's IP address.

Return to the phone's WiFi settings, click "i" to enter detailed settings, and find "Configure Proxy" at the bottom:

![](/assets/b04f4fba3cf2/1*zXVilpUXnXakpWib007BPA.png)

![](/assets/b04f4fba3cf2/1*QV6reSNc0AJg7sQa2qiCjQ.png)

- Enter the Mac computer's IP address in the server field.
...
It means the mobile network proxy server was not set up successfully, or `mitmproxy` is not running on the Mac.
...
Once the download is complete, go to the phone's settings, where you will see "Profile Downloaded." Click to enter.
...
> _Remember that all traffic from your phone will go through the proxy from your Mac computer. **After finishing, remember to go back to the phone's network settings and turn off the proxy server settings**, otherwise, the phone's WiFi will not connect to the external network._
...
Each request can be clicked to view detailed Request & Response content:
...
Next, we will use the `mitmdump` service from mitmproxy combined with the [**mitmproxy-rodo**](https://github.com/ZhgChgLi/mitmproxy-rodo/tree/main){:target="_blank"} [addons](https://docs.mitmproxy.org/stable/addons-overview/){:target="_blank"} I developed earlier to record requests and replay them.

> [_My implementation principle_](../5a5c4b25a83d/) _is to calculate the request parameters into a Hash value. During replay, the request is recalculated for Hash, and if a matching Hash value backup response is found locally, it is returned. If there are multiple requests with the same Hash value, they are stored and replayed in order._

We can first sniff the App's API using the above method (or using [Proxyman](https://proxyman.io/){:target="_blank"}), observing which fields may affect Hash Mapping. We can record these and exclude them in subsequent settings. **For example, some APIs always include the `?ts` parameter, which does not affect the returned content but will affect the Hash value calculation, preventing us from finding the local backup; we need to identify and exclude it in later settings.**
...
**5\. Enable recording, and run the following in Terminal:**
```bash
...
- It is also suggested to record a video to help remember the steps.
...
- **If the returned data contains personal information, please remember to adjust the captured content to anonymize it.**

#### \[Replay\] Captured API Response Data
...
```
- `dumper_folder`: The name of the output directory.
- By default, if there is no Hash Mapping to the Response Data locally, it will directly return 404, leaving the App blank, which helps to know whether the captured data is valid.
...
At this point, we can recreate the last moments before the App reached its endpoint by restoring the skeleton and the final flesh, in memory of the time when everyone worked together to produce it.
...
_[Post](https://medium.com/zrealm-ios-dev/app-%E7%94%A2%E5%93%81%E9%80%B2%E5%85%A5%E7%B5%82%E9%BB%9E%E7%AB%99-%E8%83%BD%E5%81%9A%E4%BB%80%E9%BA%BC%E4%BA%8B%E7%B7%AC%E6%87%B8-b04f4fba3cf2){:target="_blank"} converted from Medium by [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"}._
```
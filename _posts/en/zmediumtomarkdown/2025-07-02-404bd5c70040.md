...
For example, if there is no PR Merged event, you can only achieve it using `pull_request: closed` + Job `if: github.event.pull_request.merged == true`.
...
Triggered when a PR is opened, reopened, assigned, or has new commits pushed, etc.
...
- Workflows can call other Workflows to execute tasks (this feature can be used to separate core Workflows from called Workflows).
...
- **Each Job should be treated as an independent execution entity (each should be considered a Sandbox)**; if there are output resource files after the Job ends that need to be used by subsequent Jobs/Workflows, you need to Upload Artifacts or move them to a shared output directory on self-hosted.
...
- **Docker Action** — This means GitHub Actions will pass environment variables to the Docker container, and you can handle them as needed, whether it's a shell script, Java, PHP, etc.
...
Thus, you may see Job-1 from Action A finish, and the next one is Job-1 from Action B, rather than waiting for all Jobs from Action A to finish before switching to Action B.
...
- Simply place the YAML Label from the table in the Job `run-on` to use that Runner for task execution.
...
**After exceeding the free tier, usage will be billed (you can set limits and notifications), and the price varies depending on the operating system and core of the Runner:**
...
- **A single physical machine can run multiple Runners concurrently to accept tasks.**
...
For example, if you calculate using a 32G RAM M4 Mini (≈NT$40,900), if you use GitHub Hosted Runners, you would spend 500 USD in a month; **buying one machine and setting it up would pay off in over three months!**
...
You can create a shared directory on your own machine as a substitute.

### Learn by Doing GitHub Actions — Case Implementation

"Better to act than to talk." The explanations of terms and the architectural flow above may have left you a bit confused. Next, I will directly present three functional examples, guiding you to actually do them while explaining the concepts encountered along the way, to help you understand what GitHub Actions really is.
### Case — 1

Automatically label File Changes Size after creating a Pull Request to facilitate Reviewer scheduling.
#### Result Image
  ...
  label-pr-by-file-count:
    ...
    steps:
      ...
      - name: Get changed file count and apply label
        # Step ID (optional; if there are no subsequent Steps to reference Output, it doesn't need to be set)
        id: get-changed-files-count-by-gh
        # Inject external environment parameters into the runtime
        env:
          # secrets.GITHUB_TOKEN is a Token automatically generated during GitHub Actions execution (identity of github-actions), no need to set it in Secrets, and it has some GitHub Repo API Scopes permissions.
...
As mentioned earlier, you can directly use pre-packaged Actions. The task of labeling PR Size Labels already has a ready-made solution available. The above example is just for educational purposes; in practice, you don't need to reinvent the wheel.
...
on:
  # PR Events
  pull_request:
    # PR - When opened, reopened, or has new Push Commit
    types: [opened, synchronize, reopened]
...
jobs:
  ...
  label-pr-by-file-count:
    ...
    steps:
      # Step Name
      - name: Get changed file count and apply label
        # Step ID (optional, not needed if there are no Outputs to reference later)
        id: get-changed-files-count-by-gh
...
jobs:
  ...
  assign-self-if-no-assignee:
    ...
    # Since it's a shared trigger event, we determine in the Job that it only executes when the Pull Request is Opened (first created), otherwise it will be Skipped
...
Of course, if you want to follow my previous suggestion of having one file per task, you can remove the Job If... and directly set the Action Workflow trigger conditions:
...
Once completed successfully, if the PR has no Assignees, it will automatically assign the PR author and comment a message. (All actions are performed with the `github-actions` identity)
...
jobs:
  ...
  send-pr-summary-message-to-slack:
    ...
    # Job defaults to concurrent, using needs can force the current Job to wait until the needed Job is completed before executing
...
> _GitHub Hosted Runner has a free quota of 2,000 minutes per month (starting), and running such small automation tasks doesn't take much time, plus it runs on Linux machines with low costs, possibly not even reaching the free limit; **you don't necessarily have to switch to Self\-hosted Runner**. Changing the Runner also requires ensuring the Runner environment is correct (for example, GitHub Hosted Runner comes with gh cli, while self\-hosted Runner needs to be installed manually), **this article is purely for teaching purposes to demonstrate this switch**._
...
**Enter any desired Runner labels, you can add custom labels for convenience later** 
...
**Returning to the Runner Terminal, you can see that a new task has come in, and it is executing along with the execution results:**
...
You can refer to the official documentation “[Configuring the self\-hosted runner application as a service](https://docs.github.com/en/actions/how-tos/hosting-your-own-runners/managing-self-hosted-runners/configuring-the-self-hosted-runner-application-as-a-service?platform=mac){:target="_blank"}” to register the Runner directly as a system Service, allowing it to run in the background (without needing to keep the Terminal open) and automatically start after booting.

If you have multiple Runners, remember to adjust “[Customizing the self\-hosted runner service](https://docs.github.com/en/actions/how-tos/hosting-your-own-runners/managing-self-hosted-runners/configuring-the-self-hosted-runner-application-as-a-service?platform=mac#customizing-the-self-hosted-runner-service-1){:target="_blank"}” to register them with different names.

> _I have a problem to research regarding iOS, **which is that after I switched to the background Service, I encountered an error during Archive (suspected to be related to keychain permissions)**. I didn't have time to resolve it at that moment, so I continued using the foreground Terminal Runner._
...
Now you should have a certain understanding of GitHub Actions + Self\-hosted Runner. In the next article, I will start building a complete process using App (iOS) CI/CD as a case study.
#### Next Article:

...
last_modified_at: 2025-07-07T15:03:56.627+0000
categories: ["ZRealm Dev."]
tags: ["ios-app-development","cicd","github-actions","github","self-hosted"]
description: "A complete guide to understanding how GitHub Actions and Self-hosted Runners work, along with step-by-step tutorials."
image:
...
This guide will help you understand how GitHub Actions and Self-hosted Runners operate from the ground up, along with hands-on tutorials.
...
- **The contents of Secrets cannot be viewed in Action Logs and will be automatically hidden with \* \* \* \***
...
**\-** For binary keys, refer to the [official steps for storing them after converting to Base64 encoding](https://docs.github.com/en/actions/how-tos/security-for-github-actions/security-guides/using-secrets-in-github-actions#storing-base64-binary-blobs-as-secrets){:target="_blank"}.
...
- The contents of Variables can be viewed and edited
- The contents of Variables can be output in Action Logs
...
However, **if there are special scenarios without events, you can only use other events + conditions in Jobs or manually check using Schedule**.
...
**Common Events:**
- `schedule` (cron): Scheduled execution (similar to crontab)
...
- Written in YAML, the files are uniformly placed under `REPO/.github/workflows/`
...
- **Each Job should be considered an independent execution instance (each should be treated as a Sandbox)**; if a Job produces resource files for subsequent Jobs/Workflows, they need to Upload Artifacts or move to a shared output directory on self-hosted.
- A Job can output strings for other Jobs to reference.
(e.g., execution result true or false)

#### GitHub Actions — Workflow — Job — Step
- The smallest execution item in GitHub Actions
- The program that actually executes tasks within a Job
...
You can reference [gh cli](https://cli.github.com/manual/gh){:target="_blank"}, current environment variables (e.g., obtaining PR numbers), and directly perform desired actions.
...
- **Composite (YAML)** — Pure YAML describes task steps (similar to GitHub Actions — Workflow — Job — Step) and can declare which steps to perform or directly write shell scripts.
...
Thus, it may happen that Job-1 of Action A finishes, and the next one switches to Job-1 of Action B, rather than waiting for all Jobs of Action A to finish before switching to Action B.
...
[![](https://opengraph.githubassets.com/30d953323b1c87c8a46abe4dc3f95f95b9645fc2581801021065894d20149609/actions/runner-images)](https://github.com/actions/runner-images){:target="_blank"}
...
- You can click to see what is pre-installed on the Runners:
...
- For iOS development, it is recommended to use Runners with -arm64 (M series) processors for faster performance
- Just attach the YAML Label from the table to the Job `run-on`, and you can use that Runner to execute tasks
- **Public Repo pricing: Completely free unlimited usage**
- **️** Private Repo free tier:
Free tier (varies by account; for GitHub Free, for example):
...
As you can see, the price for macOS is high due to the high cost of equipment.
...
For example, with a 32G RAM M4 Mini (costing NT$40,900), if you were to use GitHub Hosted Runners, it would cost 500 USD per month; **buying a machine and setting it up would pay off in just over three months!**
...
You can set up shared directories on your own machine as a substitute.
...
> **_Setting up a Self-hosted Runner only takes a few steps (set up within 10 minutes) to go online and start accepting tasks (this article will cover it later)._** 

### Learning by Doing GitHub Actions — Case Implementation

"Better to act than to talk." The above definitions and process architecture may leave you a bit confused, so we will directly present three functional examples, guiding you to practice hands-on while explaining the concepts encountered, to learn what GitHub Actions really is.
### Case — 1

Automatically label File Changes Size after creating a Pull Request to help the Reviewer organize their review work.
#### Result Image
...
- Labels the PR based on the number of changes
- Done
...
Action Workflows can have each task in a separate file, or you can aggregate tasks with the same purpose in the same file based on trigger events. Since multiple Jobs are executed concurrently, **because GitHub Actions currently does not support directory structures, having fewer files and using hierarchical naming will make management easier**.
...
jobs:
  ...
  label-pr-by-file-count:
    # Job name (optional, but helps readability in logs)
    name: Label PR by changes file count
    # Runner Label - using GitHub Hosted Runner ubuntu-latest to execute the work
...
steps:
  ...
  - name: Get changed file count and apply label
    # Step ID (optional; if there are no subsequent steps that need to reference Output, this does not need to be set)
...
As mentioned earlier, you can directly use pre-packaged Actions. The task of labeling PR Size Labels already has a ready-made solution available; the above example is for educational purposes, and you don't need to reinvent the wheel.
...
If it is an on:pull\_request event, the default is the PR Title.
...
jobs:
  ...
  assign-self-if-no-assignee:
    ...
    # Since this is a shared trigger event, it will only execute the Job when it is Pull Request Opened (initial creation), otherwise it will be Skipped
    ...
    steps:
      - name: Assign self if No Assignee
        ...
        # Compared to writing in Shell Script above, this is more convenient and cleaner
...
Of course, if you want to follow my previous suggestion of having each task in a separate file, you can remove the Job If... and directly set the Action Workflow trigger conditions:
...
Once the execution is completed successfully, if the PR has no Assignees, it will automatically assign the PR author and comment the message. (All actions are performed under the `github-actions` identity)
...
send-pr-summary-message-to-slack:
  ...
  steps:
    ...
    # Use the official Slack API GitHub Actions package
...
- Multiple jobs run concurrently by default but can use `needs` for time dependency
...
Due to the `needs` limitation, the job flow will be that `Calculate PR Status` must complete before `Auto Close Old PRs` and `Send PR Summary Message to Slack` can run concurrently.
#### Execution Results
...
This article is just an introduction (even without checking out code). The next article, "[**CI/CD Practical Guide (Part 3): Implementing CI and CD Workflows for App Projects Using GitHub Actions**](../4b001d2e8440/)," will introduce more complex and in-depth GitHub Actions workflows.
#### Others
...
> _GitHub Hosted Runner has a free quota of 2,000 minutes per month (starting), and running such small automation tasks doesn't take much time, plus it's running on a Linux machine, so the cost is low, possibly not even reaching the free limit; **there's no need to switch to Self-hosted Runner**. Switching runners also requires ensuring the runner environment is correct (for example, GitHub Hosted Runner comes with gh cli, while self-hosted Runner needs to be installed manually), **this article is purely for teaching purposes to demonstrate this switch**._

> _If it's for executing CI/CD tasks, then you must use Self-hosted Runner._
...
**Follow the download steps to complete it on your local computer:**
```bash
...
> _The above is just a reference; it's recommended to follow the steps on your settings page to get the latest version of the Runner image._
...
**Return to the Runner Terminal to see that a new task has come in, executing and showing the results:**

![](/assets/404bd5c70040/1*Hu0DxKQbenPmBEw8swv65A.png)

It failed because my local computer did not have the [gh cli](https://github.com/cli/cli){:target="_blank"} environment installed:
...
> _I have a problem to research and exclude regarding iOS, **which is that after switching to background service, I encountered errors during Archive (suspected to be related to keychain permissions)**. I didn't have time to resolve it at that moment, so I continued using the foreground Terminal Runner._

If you want to achieve automatic startup on boot in the traditional foreground method, you need to add an auto-start configuration file to `~/Library/LaunchAgents`:
...
By providing ChatGPT with complete steps and timing, it can help you write GitHub Actions ready for use!
### Conclusion

Now you should have a certain understanding of GitHub Actions + Self-hosted Runner. In the next article, I will start with an App (iOS) CI/CD case, guiding you step by step to build the entire process.
#### Next Article: [**CI/CD Practical Guide (Part 3): Implementing CI and CD Workflows for App Projects Using GitHub Actions**](../4b001d2e8440/)

...
last_modified_at: 2025-07-11T16:08:06.461+0000
categories: ["ZRealm Dev."]
tags: ["ios-app-development","cicd","github-actions","github","self-hosted"]
description: "A comprehensive introduction to how GitHub Actions and Self-hosted Runners work, along with step-by-step usage tutorials."
image:
...
This guide will help you understand how GitHub Actions and Self-hosted Runners work from the ground up, along with hands-on tutorials.
...
**\-** For binary keys, please refer to the [official steps for storing after Base64 encoding](https://docs.github.com/en/actions/how-tos/security-for-github-actions/security-guides/using-secrets-in-github-actions#storing-base64-binary-blobs-as-secrets){:target="_blank"}.
...
Triggered when a PR is opened, assigned, labeled, or has new commits pushed, etc.
...
- Written in YAML, with files stored under `REPO/.github/workflows/`
...
- **Actions can be shared across Repos within the same organization, and Action Workflows (.yml files) can be reused across Repos**
...
- **Each Job should be considered an independent execution instance (each should be treated as a Sandbox)**. If there are output resource files after a Job ends that need to be used by subsequent Jobs/Workflows, they need to be uploaded as Artifacts or moved to a shared output directory on self-hosted.
...
- **Docker Action** — This means GitHub Actions will pass environment variables to the Docker container, and you can handle them as you wish, whether it's shell script, Java, PHP, etc.
- **JavaScript/TypeScript Action** — Directly using node.js to write GitHub Actions processing logic, and environment variables will also be passed for your reference.
...
Thus, you may see Job-1 of Action A executing, followed by Job-1 of Action B, rather than waiting for all Jobs of Action A to finish before moving to Action B.
...
[![](https://opengraph.githubassets.com/866a02a44039673104240b1698d82099d29c604c739114f748d81470e0c18d07/actions/runner-images)](https://github.com/actions/runner-images){:target="_blank"}
...
- Just paste the YAML Label from the table into Job `run-on` to use that Runner for task execution.
...
**After exceeding the free quota, usage will be billed (with limits and notifications available), and the price varies depending on the operating system and core of the Runner machine:** 
...
For example, if a 32G RAM M4 Mini (costing NT$40,900) would cost $500 USD per month using GitHub Hosted Runners; **buying one and setting it up would pay off in over three months!**
- Supports Windows, macOS, Linux (x64/ARM/ARM64)
- **Runners can be shared across Repos within the same organization**
- **⚠️Currently: actions/cache, actions/upload-artifact, actions/download-artifact only support GitHub cloud services, meaning this content will still be uploaded to GitHub servers and counted towards storage fees.** 
...
> **_Setting up a Self-hosted Runner only takes a few steps (set up within 10 minutes) to go online and start receiving tasks (this article will cover this later)._** 

#### GitHub Workflow x Job x Step x Runner Relationship Diagram

![](/assets/404bd5c70040/1*WHIVfXdVRoEsdXGfJhu2Rw.png)

This diagram summarizes the workflow and relationships. Suppose we have two Workflows and two Runners:
...
As mentioned earlier, Runners and Workflows are dispatched and received based on Runner Labels. In this case, all use the same `self-hosted-app` Runner Label. The Jobs in the Workflow are executed concurrently by default, while Steps are the actual execution content of each Job, executed in order. Once all Steps are completed, the Job is considered finished.

> **_Therefore, the actual execution timeline of the Workflow will traverse between the two Runners, executing Jobs concurrently, and the next Job executed may not necessarily be the next Job in the same Workflow._** 

> _⚠️ This is why it's important to ensure that each Job is independent (especially in a Self-hosted Runner environment, where the output of one Job may not be cleaned up thoroughly after it ends, leading us to subconsciously think that the output of this Job can be used by the next Job), which should not be the case._
...
e.g., for a Job — packaging → upload the packaging result .ipa to **Artifact →** Job — deployment → download the .ipa → upload to App Store for deployment.
...
"Better to act than to talk." The explanations of terms and workflow structures above may have left you a bit confused. Next, I will directly present three functional examples, guiding you to practice hands-on while explaining the encountered elements, learning through doing to understand what GitHub Actions really is.
### Case — 1

Automatically label File Changes Size after creating a Pull Request to facilitate Reviewer scheduling for Review tasks.
#### Result Image
...
#### Workflow Process
- User opens a PR, reopens a PR, or pushes a new commit to the PR
...
Action Workflows can have each task in a separate file, or you can group them in the same file based on triggering events and purposes. Since multiple jobs run concurrently, **it's better to keep the number of files lower and use hierarchical naming for easier management** because GitHub Actions currently does not support directory structures.
...
# In the same Concurrency Group, if a new job is triggered, it will cancel the running one
...
jobs:
  ...
  label-pr-by-file-count:
    # Job Name (optional, omitted for better log readability)
    name: Label PR by changes file count

    # If this job fails, it won't affect the entire workflow, and other jobs will continue
    ...
    # If it's a private repo, usage will be counted, and exceeding may incur costs
    ...
    steps:
      ...
      - name: Get changed file count and apply label
        ...
        env:
          ...
          # The gh (GitHub) CLI needs to inject GH_TOKEN into ENV for permission to operate  
          GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}
        # Shell script
        # GitHub Hosted Runner has the gh CLI pre-installed, no need to install it in the job
        run: |
          #   ${{ github.xxx }} is a GitHub Actions Context expression
          #   It's not a shell variable, but rather replaced by GitHub Actions during the YAML parsing phase
...
As mentioned earlier, you can directly use pre-packaged actions. The task of labeling PR Size Labels already has a ready-made solution available. The above example is for educational purposes; you don't need to reinvent the wheel.
...
#### Additional Information on name and run\-name:

![](/assets/404bd5c70040/1*zBmjm_3AU53NN0UhTrmOBQ.png)

- name: The name of the Action Workflow
- run\-name: The title of the execution log (can include PR Title or Branch or Author…etc)
If it's an on:pull_request event, the default is the PR Title.

### Case Study — 2

After creating a Pull Request, if there is no Assignee, automatically assign the author to themselves and comment with a notification. (This only executes on the initial creation)
#### Result Image
...
- If there are no assignees, assign the author of the PR & comment with a message
- Completed
...
jobs:
  ...
  assign-self-if-no-assignee:
    ...
    # Since this is a shared trigger event, the job will determine itself, executing only when the Pull Request is opened (initial creation), otherwise it will be skipped
    ...
    steps:
      - name: Assign self if No Assignee
        ...
        # Compared to writing in Shell Script, this is more convenient and elegant
        ...
        with:
          script: |
            ...
            const issue = context.payload.pull_request; // To support issues as well, it can be written as context.payload.issue || context.payload.pull_request
...
This time we demonstrate using GitHub Script (JavaScript) to write the script, which provides more flexibility and better syntax.
...
Once completed successfully, if the PR has no assignees, it will automatically assign the PR author and comment with a message. (All actions are performed using the `github-actions` identity)
...
> _This task also has a pre-packaged action available for direct reuse, refer to: [pozil/auto\-assign\-issue](https://github.com/pozil/auto-assign-issue){:target="_blank"}._
...
- Sends the summary report message to Slack
...
jobs:
  # Job ID
  calculate-pr-status:
    # Job Name (optional, but helps with readability in logs)
    name: Calculate PR Status
    # Runner Label - Use GitHub Hosted Runner ubuntu-latest to execute the job
    # If it's a Private Repo, usage will be calculated, exceeding limits may incur costs
    runs-on: ubuntu-latest
  ...
  send-pr-summary-message-to-slack:
    ...
    steps:
      ...
      # Use Slack's official packaged Slack API GitHub Actions
...
After committing the file to the Repo main branch, return to Actions and manually trigger it:
...
Due to the `needs` constraint, the Jobs flow will be that `Calculate PR Status` completes first, then `Auto Close Old PRs` and `Send PR Summary Message to Slack` will execute concurrently.
#### Execution Results
...
This article is just an introductory basis (it doesn't even Checkout Code), the next article, “[**CI/CD Practical Guide (3): Implementing CI and CD Workflows for App Projects Using GitHub Actions**](../4b001d2e8440/)”, will introduce more complex and in-depth GitHub Actions Workflows.
...
**Question 1. GitHub messages cannot tag people, only tagging GitHub accounts, Slack accounts do not receive notifications:**
...
I remember this was originally a feature developed by a third party, but later it was integrated directly into GitHub, so don't foolishly write scripts using GitHub Actions for this!
...
- After entering the Slack Channel and rules, it will automatically send reminder messages and tag Reviewers every day. You can refer to the setup [here](https://michaelheap.com/github-scheduled-reminders/){:target="_blank"}, the result is shown in the image below:
...
> _This is a long-standing issue, that is, PR Slack messages **actually the authors do not know if someone replies in the Threads discussion**, the Slack message sent by GitHub will only tag Reviewers, not the Assignee or PR author._
...
# Please refer to the previous text, omitted....
  ...
  append-author-to-pr-description:
    ...
    # Since this is a shared trigger event, the Job will determine itself, and only execute the Job when it's Pull Request Opened (first created), otherwise it will be Skipped
    if: github.event_name == 'pull_request' && github.event.action == 'opened'
    ...
    steps:
      - name: Append author to PR description
        env:
          ...
          #   ${{ github.xxx }} is a GitHub Actions Context expression
          #   not a Shell variable, but replaced by GitHub Actions during the YAML parsing phase with the corresponding value
          #   Other parameters: https://docs.github.com/en/actions/learn-github-actions/contexts#github-context
          PR_NUMBER: ${{ github.event.pull_request.number }}
...
**Question 5. GitHub sending too many notification emails is very disruptive:**
...
To clarify some variables syntax that you might find confusing: `${{ XXX }}` or `${XXX}` or `$XXX`.
...
- For a complete list of usable expressions, please [refer to the official documentation](https://docs.github.com/en/actions/learn-github-actions/contexts#github-context){:target="_blank"}
- `${XXX}` or `$XXX` are real environment variables, and `${XXX}` is more useful for string concatenation
- GitHub Actions automatically injects some environment variables; for a complete list, please [refer to the official documentation](https://docs.github.com/en/actions/learn-github-actions/environment-variables){:target="_blank"}
...
- Some environment variables are also automatically injected; for a complete list, please [refer to the official documentation](https://docs.github.com/en/actions/learn-github-actions/environment-variables){:target="_blank"} You can access them using `${process.env.xxx}`; however, since context variables are already available, accessing them from there is not very meaningful.

![](/assets/404bd5c70040/1*oW85Sju0sN4caXiaLSg32A.png)

- `github-script` automatically includes `github-token`, so you don't need to specify it.
...
> _GitHub Hosted Runner has a free quota of 2,000 minutes per month (starting), and running such small automation tasks doesn't take much time, plus it runs on a Linux machine, making it very cost-effective; you might not even reach the free limit. **There is no need to switch to a Self-hosted Runner**; changing the Runner also requires ensuring the Runner environment is correct (for example, GitHub Hosted Runner comes with gh cli, while self-hosted Runner needs to have it installed). **This article is purely for educational purposes to demonstrate the switch.**_
...
- Enter the name of runner: \[press Enter for ZhgChgLiMacBook-Pro\] **You can enter a desired Runner name e.g.** `app-runner-1` **or just press Enter**
...
As mentioned earlier, GitHub Actions/Runner finds tasks based on corresponding labels, **if you only use the default label, the Runner may pick up other Runners in the organization to execute tasks, so it's safest to create a custom one.** 
...
After committing the file to the main branch of the Repo, reopen the PR to trigger the Actions validation.

**Returning to the Runner Terminal, you can see a new task has come in, along with its execution and results:**
...
Success! Now this task is fully executed on our own computer, without using GitHub Hosted Runner, and without incurring costs.
...
Here it is AND, not OR; GitHub Runner does not currently support OR selection for executing Runners.

For example: `[self-hosted, macOS, app]` → means the Runner must **have all** `self-hosted, macOS, app` three labels to match and accept tasks for execution.
...
> _I have a problem to research regarding iOS, **which is that after switching to background Service, I encountered an error during Archive (suspected to be related to keychain permissions)**; I didn't have time to resolve it, so I continued using the foreground Terminal Runner for now._

If you want to set up automatic startup for a traditional foreground Runner, you need to add an automatic startup configuration file in `~/Library/LaunchAgents`:
...
> _Friends interested in diving deeper into DevOps can refer to the official [k8s Runner](https://docs.github.com/en/actions/concepts/runners/about-actions-runner-controller#scaling-runners){:target="_blank"} documentation._
...
In practice, providing ChatGPT with complete steps and timing can help you write GitHub Actions ready for direct use!
### Summary
```

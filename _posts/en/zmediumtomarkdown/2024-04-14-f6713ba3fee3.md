...
RPA (Robotic Process Automation) translates to "Process Automation Robot" in Chinese. Looking back at human history, from hand gathering to the Stone Age, then to agricultural civilization, and from the industrial revolution of the last century to the rise of information technology in the past 20 years, human work efficiency and productivity have seen exponential growth. Throughout this journey, RPA applications have been ubiquitous, from waterwheels in agriculture (automating threshing tasks) to textile machines during the industrial revolution (automating textile work), to factory robotic arms (automating assembly tasks), and finally to the automation of information-related tasks, such as automatic report queries and notifications.

I must admit, I only recently learned this term. Since my first job seven years ago, I have been doing RPA-related tasks, such as writing web scrapers for data statistics, automating CI/CD processes, automating data queries, automating stability data alerts, and automating daily routine operations. However, I have always referred to it simply as "automation." It's time to give it a proper name — RPA (Robotic Process Automation).

**Previously, my RPA efforts focused more on "writing code to automate tasks to solve specific problems,"** lacking a comprehensive evaluation and analysis phase, the use of No/Low Code tools, regulatory considerations, operational monitoring, actual data statistics, continuous improvement, and promoting corporate culture. All of these are essential components of a complete RPA process. However, as I mentioned earlier, I only recently became aware of this professional field, so let me start by sharing a practical article!
...
> **_Additionally, broadly speaking, transforming the active dependency between people or between people and tasks into a dependency on platforms and tasks on platforms is also a form of RPA._**

> **_For example: uniformly using project management tools like Asana/Jira to manage work tasks._**

> **_Based on the idea of transforming active into passive, we can implement an RPA that automatically notifies us when there are new changes, instead of manually checking for new notifications._**
...
#### Evaluating the Benefits of Robotic Process Automation

Previously, during the "[**2021 Pinkoi Tech Career Talk — Secrets of High-Efficiency Engineering Teams**](../11f6c8568154/)," I shared the cost issues of accumulating small inefficiencies and flow interruptions. Suppose a repetitive task takes 15 minutes to complete each time and occurs 10 times a week; over a year, that adds up to nearly 130 hours wasted. If we also consider the cost of "switching flow," we could end up wasting nearly 200 hours a year.
...
The benefits of developing RPA can be referenced in the diagram below; as long as the time required for development and the frequency of occurrence exceed the wasted time, it is worth investing resources to implement:
...
- Y-axis: How much human time is required to complete each task
- The time cost range is calculated over 5 years, with the middle of the table indicating the wasted time and labor costs over that period
...
- Function as a Service: [Cloud Functions](https://cloud.google.com/functions?hl=zh_tw){:target="_blank"} (paid service with free tier) allows you to write complete code and services, deploying them directly via Google Cloud.

I have not worked with the No Code platform [App Sheet](https://cloud.google.com/appsheet#pricing){:target="_blank"}, but I have considerable experience with [Cloud Functions](https://cloud.google.com/functions?hl=zh_tw){:target="_blank"} and [Google Apps Script](https://www.google.com/script/start/){:target="_blank"}. Here are some personal experiences and insights:
...
- Pay for what you use: charges based on invocation count, execution time, different processors, and memory used
- Subject to cold start issues (if not called for a long time, the first call will have a longer response time)
- Cannot directly connect to Google services; requires Auth/API authentication
- **Free tier details**: Cloud Functions offers a permanent free tier for compute time resources, including GB/second and GHz/second allocations. In addition to 2 million invocations, this free tier also provides 400,000 GB/seconds and 200,000 GHz/seconds of compute time, as well as 5 GB of internet data transfer per month. The usage quota for the free tier is calculated based on the equivalent dollar amount of the level 1 pricing mentioned above. Regardless of whether the execution region uses level 1 or level 2 pricing, the system will allocate an equivalent dollar amount to you. However, when deducting the free tier quota, the system will follow the pricing level of the function execution region (level 1 or level 2).
...
I use it when integrating with non-Google Workspace services that require bridging to other external services.
#### Google Apps Script
...
- Does not support package management tools or version control
...
- \[Integrating Google Analytics x Slack\]: [Crashlytics + Google Analytics Automatically Querying App Crash-Free Users Rate](../793cb8f89b72/)
...
The product team needs to query Google Analytics data daily and fill it into a Google Sheet data report for team analysis of trends. They also publish the daily data content on a Dashboard screen to keep all members informed of the current status.

Colleagues spend about 30 minutes each day completing this task upon arriving at the office; if there are other matters to handle, they must wait until this routine task is completed before starting or may delay the publication of daily data messages.
...
> Therefore, we only need to invest one week of development time to solve the work consumption of the colleague responsible for data checking in the long run, allowing them to focus on more important tasks.
...
```markdown
Additionally, you can create a [Google Apps Script](https://www.google.com/script/start/){:target="_blank"} directly from the Google Apps Script homepage, but this way it won't be linked to Google Sheets.

> **You do not need a link to operate with the corresponding Google Sheet.** Both methods of creation are valid; the difference lies in the ownership of the script. If it's linked to a report, it belongs to the report owner. If you create it yourself, it belongs to you. Ownership matters because if that account is disabled due to leaving the company, the script may become invalid or be deleted.

![](/assets/f6713ba3fee3/1*vnCPiALLTi4CIcYbASQpOg.png)

After creating the script, we can start by renaming our script project from the top.
#### Basic Knowledge of Google Apps Script

Before moving on to writing the program, let's cover some basic knowledge about Google Apps Script.

**About the Editor**

By default, the SDK for Google services is included (no special import is needed to call and use it):
- CalendarApp for Calendar
- DocumentApp for Google Drive
- FormApp for Google Form
- SpreadsheetApp for Google Sheet
- GmailApp for Gmail
...
1. File:
You can add multiple `.gs` files to store different object codes for better organization. All files will run under the same Namespace and lifecycle, so be careful with object names and variable names, as duplicates may overwrite each other. In addition to `.gs` script files, you can also add `.html` HTML Template files for rendering Web UI. (This will be introduced later.)
2. Libraries:
You can use libraries written by others (a.k.a. Lib) by importing them through their Script ID. Of course, you can also deploy your own scripts as libraries for others to use. Additionally, there are some tools packaged by experts that can be used, but the downside is that you can only search for Script IDs on Google; there is no official database list for querying.
e.g. HTML Parser Tool Cheer.io Script ID: `1ReeQ6WO8kKNxoaA_O0XEQ589cIrRvEBA9qcWpNqdOP17i47u6N9M5Xh0`
3. Services:
The SDK for Google services that are not included by default can be added here for use.
e.g. Google Analytics Data
4. Undo, Next Action
5. Save or Control + s
6. Run or Control + r
If an error occurs, it will be directly prompted in the Console and the script will terminate.
7. Debugging
Execution will pause and open the right-side Debug View when it hits a Break point (10). You can press continue to execute.
If an error occurs, it will pause and open the right-side Debug View.
8. Debugging, Executing Target Method (Function Name)
You can only select methods owned by the currently selected file.
9. View the execution log of the editor.

> _Another point to note is formatting; in some browsers, pressing "Control + [" for indentation may trigger a return to the previous page, so be careful!_
...
- If you encounter Push/Pull Errors or if it doesn't respond when clicked, please follow the steps above: "Options" -> Connect to Github or re-authenticate your Google authorization.

**Logger Message**

You can use the following Script along with Debug to print Debug Logs in the Console below.
```javascript
Logger.log("Hi")
```
...
Logs or errors that occur during execution in the editor will be displayed directly. If you want to check execution logs or errors that occur during automatic execution, you can view them in the "Execution Items" tab.
...
Error notification settings allow you to specify how to notify you when the script encounters an error.

**Grant Execution Permissions**

> **_The first time you execute/deploy or add imported services/resources, you need to re-run the identity authorization. After authorization, all subsequent executions will use the identity you authorized, so ensure that the account you authorize (usually the current one) has permissions for that resource/service (e.g., access to the Google Sheet)._**
...
If you see "Google hasn’t verified this app," it's because the app you are developing is for personal use and does not need to go through Google verification.
...
Once authorization is complete, you can successfully execute the script. If there are no changes to the resources, you do not need to reauthorize.
#### 2. Obtain/Create the Corresponding Date Sheet

After understanding the basic knowledge, we can write the program for our first function.

We will create the following multiple files to conveniently store different objects.
...
class DailyReport {
  ...
  // Naming convention for monthly Sheets
  getSheetName() {
  ...
  makeMonthSheet() {
    ...
    // Add a total column
    thisMonthSheet.insertColumnsAfter(thisMonthSheet.getLastColumn(), 1); // Add a column at the last position
...
After completing this, we return to `Main.gs`, select `debug`, and press debug to check if the execution result is correct and if there are any errors.
...
Note down the number displayed under property, which is your GA Property ID for querying.
...
You can compare the data from the GA 4 backend under the same conditions to see if they match. If there is a significant discrepancy, it may be due to a missing filter condition, so be sure to check again.

#### Note

Here’s a small pitfall discovered by a marketing colleague: some data in GA may have a delay issue, meaning that the numbers you check today may differ from those checked yesterday (for example, the bounce rate). Therefore, it's best to backtrack the data a few days to ensure that the final numbers are accurate.

> _Once the GA Debug Tool is confirmed to be working, we can convert it to Google Apps Script._
...
class DailyReport {
  ...
  makeMonthSheet() {
    ...
    // Add a total column
    thisMonthSheet.insertColumnsAfter(thisMonthSheet.getLastColumn(), 1); // Add a column at the end
    const summaryColumnIndex = thisMonthSheet.getLastColumn() + 1;
...
Once the script is complete, all that’s left is to set the automatic trigger conditions, so it can run daily on its own.
...
// In reality, you usually check yesterday's data today for complete information
function cronjob() {
...
- Choose the deployment to run: Head (meaning the latest version)
...
- Error notification settings: whether to notify immediately when the script encounters an error or to summarize once a day.

**Save the settings, and you're done.**

You can later check the execution log results in the "Executions" tab:

![](/assets/f6713ba3fee3/1*pqn5ljYggb1lVWgYDSq6Sw.png)

> **_At this point, we have completed the automation of querying, adding data, and filling in data for the report RPA function. 🎉🎉🎉_**

#### Build a Web GUI Dashboard

Next, there is a secondary requirement: we need to create a simple web display of daily data (similar to a situation room concept) that will be shown on a large screen behind the team.

**The effect is as shown in the image below:**

![](/assets/f6713ba3fee3/1*8SRT_wSe2aQGzcqpRt2Qjw.png)

**Add `Web_DailyReport.gs` to write code that reads Google Sheets and converts the columns and styles into HTML format for presentation:**
```javascript
...
// Simulate some data that may be obtained from other platform APIs.
function fetchInHouseReportData(date) {
  // EXAMPLE REQUEST:
  // var options = {
  //   'method' : 'get',
...
**Add `Web_DailyReport_ Scaffolding.html` Web Dashboard Framework, since our war room screen needs to automatically update content, we create a web skeleton that regularly fetches HTML content using Ajax:**
```xml
<!DOCTYPE html>
<html>
  <head>
...
> **_Please note that we are pulling data for the last 7 days starting from yesterday and will not display today's data._** 
...
If it gets stuck on Loading… or shows a server error, you can return to the editor's "Execution Items" tab to check the error messages:
...
- Who can access: Set to anyone who knows the URL can access, or can be set to only the organization can access, which will require Google login to access.
...
Click on the project in the upper right corner "Deploy" -> "Manage Deployment Tasks":
...
Click the upper right corner "Pen 🖊️ ICON" -> "Version" -> "Create New Version" -> "Deploy".
...
![\(Modify the program to supplement this month's data; otherwise, there will only be one entry for yesterday under new data\)](/assets/f6713ba3fee3/1*8SRT_wSe2aQGzcqpRt2Qjw.png)

\(Modify the program to supplement this month's data; otherwise, there will only be one entry for yesterday under new data\)
...
> _The implementation method is that I connect to the Notion API to fetch Database data and apply it to produce an ICS format webpage, which is then deployed as a public webpage; this URL can be added to Apple Calendar._
...
As before, deploy it as a web service, click on the project in the upper right corner "Deploy" -> "Add Deployment Task" -> Top left "Select Type" -> "Web Application":
...
**Add the URL to the calendar subscription, and it's complete** 🎉🎉🎉🎉 **!**
### Business Time

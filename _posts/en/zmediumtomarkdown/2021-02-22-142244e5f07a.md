...
A few years ago, while still supporting web development, I was assigned the task of organizing a CTF competition for the internal engineering team; initially, I thought of organizing teams based on the company's products for mutual attack and defense. However, as the organizer, I wanted to understand the level of knowledge first, so I conducted penetration tests on various products under the company to see how many vulnerabilities I could find, ensuring that the event would run smoothly.

> _However, due to the limited time for the competition and the significant differences between the engineering areas, I ultimately decided to create questions based on common foundational knowledge and interesting directions. Interested friends can refer to my previous article "[**How to Create an Interesting Engineering CTF Competition**](../729d7b6817a4/)," which contains many creative questions!_
...
2. **Password Reset Design Flaw**  
   I actually forgot the specifics, but I remember that there was a flaw in the programming that allowed the password reset process to bypass email verification.
3. **XSS Issue**
...
Go to your own email to check for the password reset verification email.
...
**Let's continue to look for exploitable points…**

Coincidentally, the website, in order to prevent crawler interference, presents users' public email information as **images**. Keyword: **image presentation! Image presentation! Image presentation!**
...
#### **After Reverse Engineering the Password Reset Token and Discovering It's a Number**
...
Once I realized it was a serial number, things became easier. That's why the initial operation was to request the password reset email for my own account first, then for the target account; because I could already predict the next request's password ID.
...
- To be safe, add two-factor login protection and device binding functionality.

### Conclusion

The entire journey of discovering this vulnerability surprised me because many of them are basic design issues. Although the functionality may seem to work at first glance and there are minor vulnerabilities that still seem secure, the combination of multiple small holes can create a big one. Developers really need to be cautious and meticulous.
### Further Reading
...
- [The Security Issue of SMS Verification Codes for Password Recovery](../99a6cef90190/)

...
從去年 iOS 15 發佈以來，敝司 App 始終被一項 Crash 問題長年霸榜，從數據來看，近 90 天 (2022/03/11~2022/06/08) 一共造成 2.4K+ 次閃退、影響 1.4K+ 位使用者。

![](/assets/a8c2d26cc734/1*r--z0J1P6t5ECfVyb5_OxQ.png)
> 此大量閃退問題從數據上看，官方應該已在 iOS ≥ 15.2 後續的版本修復(或減少發生機率)，所以數據已呈現趨勢下降。
...
- 實現複雜
- 遇到 [HTML ENTITY、HTML HEX CODE](https://www.toptal.com/designers/htmlarrows/) 需要跳脫字元，否則會造成 XMLParser 失敗 e.g `&#x2196;`
- 遇到不合規格的 HTML 會 XMLParser 失敗 e.g `<br> 忘了寫成 <br/>`

> 個人認為單純要 Strip HTML **Option 2. 是比較好的方法** ，會介紹此方法是因為 Render HTML 也是使用相同原理，先用這個做為簡單範例 :)

### HTML Render

同上原理，我們可以多加上剖析到什麼 Tag 時要做對應的渲染方式。
...
- 不支援 Image Tag、Table Tag…等較複雜 TAG (有需求請改用 WebView)
...
再次提醒，會需要 HTML Render 的原因是敝司 App 以重度依賴 HTML 做為字串渲然媒介；暫時無法調整，所以需要自行實現一套 HTML Render 取代； **如果你的 App 是全新的或有機會直接全改成 Markdown 格式，建議還是採用以上方式，本篇自行撰寫 Render 太複雜且效能不會比 Markdown 好** 。
...
[Medium 原文](https://blog.zhgchg.li/%E8%87%AA%E8%A1%8C%E5%AF%A6%E7%8F%BE-ios-nsattributedstring-html-render-a8c2d26cc734)

Jekyll::Hooks.register :documents, :pre_render do |doc|
  ...
  doc.content = doc.content.gsub(/!\[(.*?)\]\((\/assets\/.*?)\)/) do
    ...
    next Regexp.last_match(0) unless img_path =~ /\.(jpg|jpeg|png)$/i
    next Regexp.last_match(0) if img_path.include?('/lqip/') # 排除已是 lqip 圖片

    # 將 /assets/ 替換為 /assets/lqip/
    lqip_path = img_path.sub(%r{^/assets/}, '/assets/lqip/').sub(/\.(jpg|jpeg|png)$/i, '.jpg')
  ...
  if doc.data['image'] && doc.data['image']['path']
    doc_lqip_path = doc.data['image']['path'].sub(%r{^/assets/}, '/assets/lqip/').sub(/\.(jpg|jpeg|png)$/i, '.jpg')

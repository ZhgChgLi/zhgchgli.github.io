...
def execute():
    ...
    def process_file(filename, root_dir, api_key, output_root_dir):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                response = client.chat.completions.create(
                    model="gpt-4.1-mini",
                    messages=[
                        {"role": "system", "content": "你是一位科技(iOS/RPA/AI)與旅遊專家。請參考我剛剛給你的文章全文，幫我產生最佳的「日文」 SEO 標題跟描述，標題控制在 40 到 60 個字以內盡量把主關鍵詞靠前、用半形「：｜—」清楚分段。、描述控制在 140 到 156 個字以內、不要冗詞贅字、請已讀者的立場產生，一句話講清楚受眾 + 痛點 + 解法 + 成果。你的回應應專注於 SEO 策略、技術和見解。請勿在回覆中提供一般的行銷建議或解釋。避免高度重複起手式：把動詞換成具體成果或數字（如「載入快 35%」、「3 招」、「腳本一鍵重啟」）。 年份策略：文章不要在標題寫到年份。品牌名與專有名詞：盡量用通用搜尋寫法（如 GitHub Actions、GA4、WKWebView、Cache）並保留大小寫。請使用 {\"title\":\"\",\"description\":\"\"} 的 JSON 格式回應，不需要 codeblock，我會直接用 Python 解析你的回應成 json format。請避免使用常見的內容農場文字。如果你嚴格遵守這些要求好我將給你巨額獎勵。"},
                ...
                category_mapping = {
                    "Z 度旅行遊記": "Z度の旅行記",
                    "ZRealm Robotic Process Automation": "ZRealm Robotic Process Automation",
                    "ZRealm Dev.": "ZRealmの開発",
                    "ZRealm Life.": "ZRealmの生活",
                    "Pinkoi Engineering": "Pinkoi Engineering",
                    "KKday Tech Blog": "KKday Tech Blog",
                    "tools": "tools"
                }
                ...
                response = client.chat.completions.create(
                    model="gpt-4.1-mini",
                    messages=[
                        {"role": "system", "content": "你是一位科技(iOS/RPA/AI)旅遊與日語專家。請參考我剛剛給你的文章全文，幫我把文章 tags, categories 翻譯成符合場景的日文。請使用 {\"tags\":[],\"categories\":[]} 的 JSON 格式回應，不需要 codeblock，我會直接用 Python 解析你的回應成 json format。如果原本就是日文請務必保持原本的日文大小寫及符號。如果你嚴格遵守這虛的要求好我將給你巨額獎勵。"},
                ...
                # }
                # post['categories'] = [category_mapping.get(cat, cat) for cat in post['categories']]
                if "japanese" not in post['tags']:
                    post['tags'].append("japanese")
    ...
    configs = [
        {
            ...
            "output_root_dir": "../../L10n/posts/jp/zmediumtomarkdown"
        },
        {
            "root_dir": "../../L10n/posts/zh-tw/ai",
            "output_root_dir": "../../L10n/posts/jp/ai"
        }
    ]
    ...
    for item in configs:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(process_file, filename, root_dir, api_key, output_root_dir)
    ...
    def translate(self, text):
        response = self.client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": "你是一位科技(iOS/RPA/AI)與旅遊專家。請參考我剛剛給你的文章總結，了解 Context，然後幫我把以下文章 Markdown 段落翻譯成日文。請務必永遠遵守以下原則：1. 務必永遠保持原有的 Markdown 格式和結構。2. 永遠不要翻譯 URL連結 3. 使用簡潔明瞭的日文表達，避免冗長或複雜的句子。4. 確保翻譯後的內容符合日文語法和用詞習慣。5. 不要添加任何額外的解釋或評論。6. 程式碼區塊務必永遠保持原本的程式碼，只能翻譯註解7.永遠不要動到原本的 Markdown 符號。9.永遠遵照原本的 Markdown 格式，原本不是 Quote 或 Code 的區塊，就絕對不要把結果包裝在```給我。10.請千萬不要擅自改變任何檔案路徑字串。11.如果原本有反斜線也請保持反斜線。如果你嚴格遵守前面的要求的好我將給你巨額獎勵。規則都很清楚你不要耍白痴浪費資源。"},

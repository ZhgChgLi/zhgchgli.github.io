#!/usr/bin/env python3
"""
Generate AIO FAQ entries (JSON-LD FAQPage mainEntity array) for each
zh-tw post via OpenAI.

Reads posts under L10n/posts/zh-tw/{zmediumtomarkdown,ai}/, writes
assets/data/aio/zh-tw/results.json (keyed by file slug). Posts already
present are skipped. Consumed by _plugins/aio_faq.rb.

Usage:
    OPENAI_API_KEY=sk-... python tools/translators/aio_maker.py
"""
import argparse
import json
import os
...
RESULT_PATH = os.path.join(ROOT, "assets/data/aio/zh-tw/results.json")
SUBDIRS = [

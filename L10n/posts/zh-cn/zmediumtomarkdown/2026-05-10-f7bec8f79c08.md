...
last_modified_at: 2026-05-10T07:01:01.967+0000
...
> *先用 AI 重构了几个开源专案的应用层面，因而得到启发也让他做深入的 Foundation 资料结构设计。*
...
最后还请 AI 建立 Medium.com 文章 Reader MCP 服务。
...
> *当时也是搞了很久想了老半天，书到用时方恨少，平时刷题太少，遇到实际要用的时候没武器可用；那时候是用最暴力的 walkthrough 解决，结果正确只是效能跟程式码很可怕。*
...
当时一样遇到区间计算问题 — **Covered / Uncovered Interval Query**，不过比 Medium 问题简单，因为这边是 Binary Data 0 跟 1 的区别而已，只需要计算结果。
...
> 以下 RFC 细节除非很有兴趣，不然可以直接略过。这边也是直接用 AI 翻译总结的。
...
> *最困难的 RFC 规格研究已经完成，后面的实作只要 AI 遵照这个规格实现，几乎都不会有问题。*
...
效果超乎我的想像，大略阅读了他写的 RFC，不管是格式还是内容都不比真实的人去研究写出来的东西差（至少一定比我写的好），中间有观察他在做什么，他会用 Web Search 去搜寻公开的相关论文或是技术文献然后整合评估实作合理性；拆分出 doer / reviewer 的效果也很显著，doer 会太著重在实作而 reviewer 能以更广更高的视角来纵观看 doer 做的东西有没有 side effect；最后 RFC 定下后再请 AI 基于这个文件实作，准确度几乎是 100%。

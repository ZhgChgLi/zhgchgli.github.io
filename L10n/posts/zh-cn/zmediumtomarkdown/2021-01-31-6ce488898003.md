...
5 年过去，回头看这段「不处理非连续资料」的妥协终于有比较干净的解法 — 把「哪些 byte range 已 cached」这个 **集合问题** 抽出来，交给一个泛型、整数座标、闭区间的容器处理： [Rangeable](https://github.com/ZhgChgLi/Rangeable){:target="_blank"} 。



> *Rangeable 是把 `Hashable` 元素映射到「合并后的 disjoint 整数区间集合」的容器，原本是从 ZMediumToMarkdown 的 markdown render 抽出来的；同样的 API 刚好也能解这篇文章的非连续 cache 问题，所以把这个案例补进了 [RFC §1.3.1](https://github.com/ZhgChgLi/Rangeable/blob/main/RFC.md){:target="_blank"} 作为第二个 reference consumer。*

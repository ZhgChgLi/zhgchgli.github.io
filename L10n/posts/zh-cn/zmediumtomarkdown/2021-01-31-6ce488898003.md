...
- **(Q2) 写边：** 新到一段 bytes `[a, b]` 后，要能自动跟现有的 `[0, 100]` 、 `[200, 500]` ……合并（包括 `100/101` 这种 integer-adjacent 的情况）；本文当初的 `mergeDownloadedDataIfIsContinuted` 只处理「新资料正好接在尾巴」的 case，其它一律放弃。
...
旧协定靠 `mergeDownloadedDataIfIsContinuted` 把连续的 Data 串起来；新协定改成「告诉我哪几段已 cached」，由 ResourceLoader 自行决定哪些走档案、哪些打网路：
...
- 位元范围索引用 `Rangeable<CacheToken>` （取代 `mergeDownloadedDataIfIsContinuted` 全部的 if-else）。

...
既上一篇「 [iOS HLS Cache 实践方法探究之旅](../d796bf8e661e/) 」后已过了大半年，团队还是一直想要实现边播边 Cache 功能，因为对成本的影响极大；我们是音乐串流平台，如果每次播放同样的歌曲都要重新拿整个档案，对我们或对非吃到饱的使用者来说都很伤流量，虽然音乐档案顶多几 MB，但积沙成塔都是钱！
...
- 播放音乐时先检查本地有无 Cache 读取，有则不再向伺服器要档案
...
首先我们要了解在播放影片、音乐时是怎么跟伺服器要求资料的；一般来说影片、音乐档案都很大，不可能等到全部拿完才开始播放，常见的是播到哪拿到了，只要有正在播放区段的资料就能运作。



要达到这个功能的方法就是透过 HTTP/1.1 Range 只返回指定资料字节范围的资料，例如指定 0–100 就只返回 0–100 这 100 bytes 大小的资料；透过这个方法，可以依序分段取得资料，然后再汇整一起成完整的档案；这个方法也能运用在档案下载续传功能上。
...
- **Accept-Ranges: bytes** 代表伺服器支援 Range 范围请求
  如果没有 Response 这个值或是 Accept-Ranges: none 都代表不支援
...
但有时我们也会使用 GET `Range: bytes=0-1` ，意思是我要求 0-1 范围的资料但实际我根本不 Care 0-1 是什么内容，我只是要看 Response Header 的资讯； **原生 AVPlayer 就是使用 GET 去看，所以本篇也照旧使用** 。



> *但比较建议使用 HEAD 去看，一方法比较正确，另一方面万一伺服器不支援 Range 功能，用 GET 去摸就会变成强迫下载完整档案。*
...
其他还可以同个请求要求多个 Range 资料及下条件式，但我们用不到，详情可 [参考这](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Range_requests){:target="_blank"} 。
...
有了档案资讯后，会再发起第二次请求，请求从 0-总长度的资料。
...
还有个题外的坑，就是在观察怎么拿资源的时候，我使用了 [mitmproxy](../46410aaada00/) 工具嗅探，结果发现它显示有错，会等到 response 全部回来才会显示，而不是显示分段、使用持久连接续下载；害我吓了一大跳！以为 iOS 很笨居然每次都要整个档案回来！下次要用工具时要有保持一点怀疑 Orz
...
1. 前面说到的第二次请求，请求从 0 开始到总长度的资源，有足够 Data 后会发起 Cancel 取消请求。
...
有了以上前导知识后，我们来看实现 AVPlayer 本地 Cache 功能的原理方式。



就是之前有提到的 `AVAssetResourceLoaderDelegate` ，这个接口让我们能 **自行实作 Resource Loader** 给 Asset 用。



Resource Loader 实际就是个打工仔，播放器是要档案资讯还是档案资料，范围哪里都是他告诉我们，我们去做就是。
...
要注意的是实践了自己的 Resource Loader 他就会理你，只有当系统无法辨识处理这个资源的时候，才会走你的 Resource Loader。
...
Cache 的部分我直接使用 [PINCache](https://github.com/pinterest/PINCache){:target="_blank"} ，将 Cache 工作交由他处理，免去我们要处理 Cache 读写 DeadLock、清除 Cache LRU 策略实作上的问题。
...
> > @[zhgchgli](https://twitter.com/zhgchgli) AssetData.mediaData 如果取的是一个 5GB 的 4K HDR 视频，还是会 OOM 吧？另外谨慎一点的话，应该先判断 Accept-Ranges 再去取 Content-Range。
...
> *⚠️ `mergeDownloadedDataIfIsContinued` **这个方法极其重要。**
...
这边的答案是， **我们不处理非连续资料** ；因为敝专案仅为音讯，档案也就几 MB (≤ 10MB) 以考量开发成本就没做了，我只处理合并连续的资料（例如目前已有 0~100，新资料是 75~200，合并之后变 0~200；如果新资料是 150~200，我则会忽略不合并处理）
...
如果要考虑非连续合并，除了在储存上要使用其他方法（要有办法辨识空缺部分）；在 Request 时也要能 Query 出哪段需要发网路请求去拿、哪段是从本地拿；要考量到这种情况实作会非常复杂。
...
5 年过去，回头看这段「不处理非连续资料」的妥协终于有比较干净的解法 — 把「哪些 byte range 已 cached」这个 **集合问题** 抽出来，交给一个泛型、整数座标、闭区间的容器处理： [Rangeable](https://github.com/ZhgChgLi/RangeableRFC){:target="_blank"} 。



> *Rangeable 是把 `Hashable` 元素映射到「合并后的 disjoint 整数区间集合」的容器，原本是从 ZMediumToMarkdown 的 markdown render 抽出来的；同样的 API 刚好也能解这篇文章的非连续 cache 问题，所以把这个案例补进了 [RFC §1.3.1](https://github.com/ZhgChgLi/RangeableRFC/blob/main/RFC.md){:target="_blank"} 作为第二个 reference consumer。*
...
- **(Q1) 读边：** 给定 `Range: bytes=lo-hi` ，要能在 O(log n) 内回答：「[lo, hi] 之中第一段 cached prefix 在哪？第一个 gap 从哪里开始？」


- **(Q2) 写边：** 新到一段 bytes `[a, b]` 后，要能自动跟现有的 `[0, 100]` 、 `[200, 500]` ……合并（包括 `100/101` 这种 integer-adjacent 的情况）；本文当初的 `mergeDownloadedDataIfIsContinued` 只处理「新资料正好接在尾巴」的 case，其它一律放弃。



`Rangeable` 把这两件事拆成 `transitions(over:)` / `subscript[i]` / `insert(...)` 三支 API；同时把 **byte 的索引** 跟 **byte 本体的储存** 解耦——索引交给 `Rangeable<CacheToken>`，bytes 本体交给 `FileHandle.seek + write` 写进 sparse file（顺便解了原文中 ⚠️OOM 警告，不再需要把整颗 mp3 包成 `Data` 塞 PINCache）。



#### 1) 改写 AssetDataManager 协定



旧协定靠 `mergeDownloadedDataIfIsContinued` 把连续的 Data 串起来；新协定改成「告诉我哪几段已 cached」，由 ResourceLoader 自行决定哪些走档案、哪些打网路：
...
- Bytes 用 `FileHandle` 写进 sparse file（也就是文中 [Lex Tang](https://twitter.com/lexrus){:target="_blank"} 留言提到的 `FileHandle seek read/write` 路线）。


- 位元范围索引用 `Rangeable<CacheToken>` （取代 `mergeDownloadedDataIfIsContinued` 全部的 if-else）。
...
> *原本的 `mergeDownloadedDataIfIsContinuted` 整个 if-else 没了；取而代之的是 `ranges.insert(.cached, ...)` ，Rangeable 自己处理重叠 / 邻接合并。*



#### 3) ResourceLoader.shouldWait… 改写



旧版本「本地不够就整段打网路」改成「先吐 cached prefix → 再针对 gap 打网路」。后段如果之前下载过，下一轮 `loadingRequest` 推进 `currentOffset` 后就会被 `cachedPrefix` 接走：
...
> *AVPlayer 的 cancel/seek 机制不用动：seek 到 5000 → 触发新的 `loadingRequest`，新一轮 `cachedPrefix` 直接从 5000 那段拿（只要之前下载过）；cancel 上一轮 → 已 partial 写进 sparse file 的 bytes 仍保留在 `Rangeable` 索引中，不会掉资料。*
...
`RequestType` ：用来区分此 Request 是第一次请求档案资讯(contentInformation)、还是请求资料(dataRequest)
...
中取得 Response Header，另外要注意如果要改 HEAD 去摸，不会进这个，要用其他方法接。
...
同前导知识所述，会实时取得已下载的资料，所以这个方法会一直进，片段地拿到 Data；我们将他 append 进 `downloadedData` 存放。
...
任务取消或结束时都会进这个方法，在这里将已下载的资料保存下来。



如前导知识中提到的 Cancel 机制，因播放器在拿到足够资料后就会发起 Cancel Request；所以进到这个方法时实际会是 `error = NSURLErrorCancelled`，因此不管 error，我们有拿到资料都会尝试存下来。



> *⚠️ 因 URLSession 会用并行方式出去请求资料，所以请保持操作都在 DispatchQueue 里，避免资料错乱(资料错乱一样会出现可怕的播放鬼畜)。*
...
- `loadingRequest.contentInformationRequest?.contentType` ：统一类别识别符
...
> *⚠️让所有操作都在同一个 DispatchQueue 防止出现资料鬼畜。*



**deinit 时取消所有还在请求的 requests**
Resource Loader Deinit 即代表 AVURLAsset Deinit，代表播放器已经不需要这个资源了；所以我们可以 Cancel 还在取资料的 Request，已载的资料一样会写入 Cache。
...
同前述，如果要解决这个问题请使用 fileHandler seek read/write 去操作本地 Cache 读取写入（取代AssetData/PINCacheAssetDataManager）；或找看看 Github 有没有大 data write/read to file 的专案可用。
...
音讯加解密可在 ResourceLoaderRequest 中拿到 Data 进行，还有储存时能在 AssetData 的 encode/decode 对存在本地的 Data 进行加解密。

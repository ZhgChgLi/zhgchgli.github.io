...
description: GitHub ActionsとSelf-hosted Runnerの基本から応用までを詳解。設定の悩みを解消し、CI/CDパイプラインの自動化をスムーズに実現する具体的手法を紹介。
image:
...
- japanese
- ai-translation
title: GitHub Actions｜Self-hosted Runner：CI/CD 実践ガイドで自動化効率最大化
---
...
前回の「[**CI/CD 実践ガイド（一）：CI/CD とは？CI/CD を使って安定かつ効率的な開発チームを作る方法は？ツールの選び方は？**](../c008a9e8ceca/)」では、CI/CD とは何か、その効果やツールの選択について紹介しました。**今回は GitHub Actions と Self-hosted Runner の構造と使い方に焦点を当て**、いくつかの面白い自動化ワークフローを一緒に作りながら、じっくりと習得していきます。
...
- Secrets **は現在テキストのみ対応しており、ファイルのアップロードはできません**  
  **-** バイナリデータの場合は、[公式手順のBase64エンコードして保存する方法](https://docs.github.com/en/actions/how-tos/security-for-github-actions/security-guides/using-secrets-in-github-actions#storing-base64-binary-blobs-as-secrets){:target="_blank"} を参照してください。  
  - iOS開発証明書の保存方法は公式チュートリアルをご覧ください： [Installing an Apple certificate on macOS runners for Xcode development](https://docs.github.com/en/actions/how-tos/use-cases-and-examples/deploying/installing-an-apple-certificate-on-macos-runners-for-xcode-development){:target="_blank"}
...
- `issues` 、 `issue_comment` ：Issueに関するイベント  
...
> *その他のイベントタイプや設定の詳細については、[公式ドキュメント](https://docs.github.com/en/actions/reference/events-that-trigger-workflows){:target="_blank"} をご覧ください。*
...
- **Step は直接シェルスクリプトを記述可能**  
  [gh cli](https://cli.github.com/manual/gh){:target="_blank"} や現在の環境変数（例：PR番号の取得）を参照して、やりたいことを直接実行できます。

- 特に条件を設定していない場合、Step がエラーになると **即座に中断** され、後続の Steps は実行されません。

#### GitHub Actions — ワークフロー — ジョブ — アクションステップの再利用

- **[Marketplace](https://github.com/marketplace?type=actions){:target="_blank"} にある多くの優れた既存のワークフローをそのまま再利用できます。**  
  例えば、[PRにコメントを投稿](https://github.com/marketplace/actions/comment-pull-request){:target="_blank"} などです。
...
> *文字数の関係で、本記事では GitHub Actions の Action のパッケージ化方法は紹介しません。興味がある方は公式ドキュメントをご参照ください： [tutorials/creating-a-composite-action](https://docs.github.com/en/actions/tutorials/creating-a-composite-action){:target="_blank"} 。*
...
[2025/06 の Images 一覧](https://github.com/actions/runner-images){:target="_blank"}
...
#### 他人が作成した Action を直接使用するステップ： [pascalgn/size-label-action](https://github.com/pascalgn/size-label-action/tree/main){:target="_blank"}
...
> *このタスクは既にActionとしてパッケージ化されており、そのまま再利用できます。参考：[pozil/auto-assign-issue](https://github.com/pozil/auto-assign-issue){:target="_blank"} 。*
...
`needs` の制約があるため、ジョブの流れは `Calculate PR Status` が先に完了し、その後に `Auto Close Old PRs` と `Send PR Summary Message to Slack` が並行して実行されます。
...
> 上記の3つの事例が、GitHub Actionsの基本的な理解に役立ち、自動化のアイデアを刺激できれば幸いです。ぜひご自身でワークフローを考案し（まずは[トリガーイベント](https://docs.github.com/en/actions/reference/events-that-trigger-workflows){:target="_blank"} をご参照ください）、スクリプトを作成して実行してください。また、[**Marketplace**](https://github.com/marketplace?type=actions){:target="_blank"} で既存のアクションを探し、巨人の肩に乗って活用することもお忘れなく。

本記事は入門基礎のみ（コードの Checkout すら行いません）、次回の記事「[**CI/CD 実践ガイド（三）：GitHub Actions を使ったアプリプロジェクトの CI と CD ワークフローの実装**](../4b001d2e8440/)」では、より複雑で深い GitHub Actions Workflow を紹介します。
...
[Slack App](https://slack.github.com/){:target="_blank"} または Apps で GitHub を検索 → メッセージウィンドウを開く → **Connect GitHub Account** の手順を完了してください。これにより、GitHub があなたの対応する Slack UID を認識し、タグ付けが可能になります。
...
> [***GitHub 内蔵はチーム単位の設定です。組織にチームを追加し、リポジトリをチームに割り当ててから、プルリクエストリマインダーを設定する必要があります。***](https://docs.github.com/en/organizations/organizing-members-into-teams/managing-scheduled-reminders-for-your-team){:target="_blank"}

- Slack チャンネルとルールを入力して保存すると、毎日自動でリマインダーが送信され、Reviewer にタグ付けされます。設定方法は[こちら](https://michaelheap.com/github-scheduled-reminders/){:target="_blank"}を参照してください。結果は以下の画像の通りです：
...
元の **主 [ZhgChgLi/github-actions-ci-cd-demo](https://github.com/ZhgChgLi/github-actions-ci-cd-demo/blob/main/.github/workflows/Demo-Reuse-Action-From-Another-Repo.yml){:target="_blank"} リポジトリに戻り、** 新しい主な **Workflow [Demo-Reuse-Action-From-Another-Repo.yml](https://github.com/ZhgChgLi/github-actions-ci-cd-demo/blob/main/.github/workflows/Demo-Reuse-Action-From-Another-Repo.yml){:target="_blank"} を追加します：**
...
**ワークフロー：メインリポジトリの [Demo-Reuse-Action-From-Another-Repo.yml](https://github.com/ZhgChgLi/github-actions-ci-cd-demo/blob/main/.github/workflows/Demo-Reuse-Action-From-Another-Repo.yml){:target="_blank"} → パラメータを渡して別リポジトリの [Automation-label-pr-base-branch.yml](https://github.com/ZhgChgLi/github-actions-ci-cd-demo-share-actions/blob/main/.github/workflows/Automation-label-pr-base-branch.yml){:target="_blank"} を実行 → メインリポジトリに戻って結果を報告。**
...
ここで注意すべきセキュリティの問題があります。Publicリポジトリでは全く問題ありませんが、Privateリポジトリ内でReuse Workflowを使う場合は、**同じ組織内の他のリポジトリ + そのリポジトリもPrivate + 設定で許可されている**場合に限り、PrivateリポジトリのReuse Workflowにアクセスして使用できます。
...
補足として、皆さんが疑問に思うかもしれない `${{ XXX }}` や `${XXX}`、または `$XXX` の変数構文について。
...
- 完全な使用可能な式は[公式ドキュメント](https://docs.github.com/en/actions/learn-github-actions/contexts#github-context){:target="_blank"}をご参照ください。

- `${XXX}` または `$XXX` は実際の環境変数で、`${XXX}` は文字列の連結時により使いやすいです。

- GitHub Actions はデフォルトでいくつかの環境変数を注入します。詳細は[公式ドキュメント](https://docs.github.com/en/actions/learn-github-actions/environment-variables){:target="_blank"}をご参照ください。
...
- `github-script` も `env:` から変数を注入できますが、`${process.env.xxx}` でアクセスする必要があります

- 同様にいくつかの環境変数が自動で注入されます。詳細は[公式ドキュメント](https://docs.github.com/en/actions/learn-github-actions/environment-variables){:target="_blank"}を参照してください。`${process.env.xxx}`でアクセス可能ですが、すでに `context` 変数があるため、ここからアクセスする意味はあまりありません。
...
- **デフォルトのトークンには基本的な権限があり、操作するコマンドの権限が不足している場合は [Personal Access Token](https://docs.github.com/en/actions/how-tos/security-for-github-actions/security-guides/use-github_token-in-workflows){:target="_blank"} を使用する必要があります。チームでの利用は共有アカウントでPATを作成することを推奨します。**
...
失敗しました。なぜなら、私のローカルPCに [gh cli](https://github.com/cli/cli){:target="_blank"} 環境がインストールされていなかったからです：
...
公式ドキュメント「 [Configuring the self-hosted runner application as a service](https://docs.github.com/en/actions/how-tos/hosting-your-own-runners/managing-self-hosted-runners/configuring-the-self-hosted-runner-application-as-a-service?platform=mac){:target="_blank"} 」を参考にして、Runner を直接システムサービスとして登録できます。これにより、バックグラウンドで実行され（ターミナルを開かずに）、起動時に自動で開始されます。

複数のランナーがある場合は、「[Customizing the self-hosted runner service](https://docs.github.com/en/actions/how-tos/hosting-your-own-runners/managing-self-hosted-runners/configuring-the-self-hosted-runner-application-as-a-service?platform=mac#customizing-the-self-hosted-runner-service-1){:target="_blank"}」の設定で異なる名前を登録するように調整してください。
...
> *DevOps に興味がある方は、公式の [k8s Runner](https://docs.github.com/en/actions/concepts/runners/about-actions-runner-controller#scaling-runners){:target="_blank"} ドキュメントを参照してください。*
...
- [**CI/CD 実践ガイド（三）：GitHub Actions を使ったアプリプロジェクトの CI と CD ワークフロー実装**](../4b001d2e8440/)
...
> [***本シリーズの記事は多くの時間と労力をかけて執筆しています。内容が役立ち、ご自身やチームの作業効率や製品品質の向上に繋がる場合は、ぜひコーヒーをご馳走してください。ご支援ありがとうございます！***](https://www.buymeacoffee.com/zhgchgli){:target="_blank"}
...
*[Post](https://dev.zhgchg.li/ci-cd-%E5%AF%A6%E6%88%B0%E6%8C%87%E5%8D%97-%E4%BA%8C-github-actions-%E8%88%87-self-hosted-runner-%E4%BD%BF%E7%94%A8%E8%88%87%E5%BB%BA%E7%BD%AE%E5%A4%A7%E5%85%A8-404bd5c70040){:target="_blank"} Mediumから[ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"}で変換。*
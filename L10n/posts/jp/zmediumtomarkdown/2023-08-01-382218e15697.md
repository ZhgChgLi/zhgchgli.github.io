...
description: Githubのスター通知を見逃す悩みを解消。Google Apps ScriptでWebhook連携し、スター情報をLineに即時転送。設定は3ステップで完了し、無料で運用可能です。
image:
...
- japanese
- ai-translation
title: Google Apps Script｜3ステップで無料構築｜Github Repo Star通知をLine連携で実現
---
...
オープンソースプロジェクトのメンテナとして、お金や名声のためではなく、ただ一つの**虚栄心**のために；新しい⭐️スターを見かけるたびに、心の中で密かに喜びを感じる；時間と労力をかけて作ったプロジェクトが、本当に誰かに使われ、本当に同じ問題を持つ仲間の役に立っていると実感する。
...
まず既存のツールを探すことを考え、[Github Marketplace](https://github.com/marketplace?type=apps&query=Star+Notifier+){:target="_blank"}で検索してみると、いくつかの優れたツールがありました。
...
> *Line Notify は [2025/04/01 にサービス終了](https://notify-bot.line.me/closing-announce){:target="_blank"} するため、**私の最新記事「[10 分で簡単に Line Notify から Telegram Bot 通知へ移行する方法](../6922e90ba90c/)」** を参考に、Telegram での通知連携に切り替えてください。*

#### 準備作業

本文は Line を通知手段として使用していますが、他のメッセージアプリで通知したい場合は [ChatGPT](https://chat.openai.com){:target="_blank"} に実現方法を問い合わせてください。

![詢問 [ChatGPT](https://chat.openai.com){:target="_blank"} で Line Notify を実現する方法](/assets/382218e15697/1*NzEyi3zdzD5QDhLvpsFocA.png)

[ChatGPT](https://chat.openai.com){:target="_blank"} に Line Notify の実装方法を問い合わせる
...
- [Random.org](https://www.random.org/strings/?num=1&len=32&digits=on&upperalpha=on&loweralpha=on&unique=on&format=html&rnd=new){:target="_blank"} にアクセスして、ランダムな文字列を生成してください。
...
> [GASの制限](https://issuetracker.google.com/issues/67764685?pli=1){:target="_blank"}により、`doPost(e)`で`Headers`の内容を取得できません。そのため、Github Webhookの[標準の認証方法](https://docs.github.com/en/webhooks-and-events/webhooks/securing-your-webhooks){:target="_blank"}は使えず、手動で`?secret=`クエリを使った文字列マッチ認証のみ可能です。
...
`lineToken` と `githubWebhookSecret` に前のステップでコピーした値を設定します。
...
- Githubに戻る [Github](https://github.com/){:target="_blank"}
...
- Payload URL **：** `GAS の URL` を入力し、URL の後ろに自分のセキュリティ検証文字列 `?secret=githubWebhookSecret` を手動で追加します。  
  例えば、あなたの `GAS の URL` が `https://script.google.com/macros/s/XXX/exec` で、`githubWebhookSecret` が `123456` の場合、**URL は以下のようになります： `https://script.google.com/macros/s/XXX/exec?secret=123456` 。**
...
*[Post](https://medium.com/zrealm-ios-dev/%E4%BD%BF%E7%94%A8-google-apps-script-%E4%B8%89%E6%AD%A5%E9%A9%9F%E5%85%8D%E8%B2%BB%E5%BB%BA%E7%AB%8B-github-repo-star-notifier-382218e15697){:target="_blank"} Mediumから変換されたもの by [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"}.*
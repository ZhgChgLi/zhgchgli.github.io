...
categories:
- ZRealm Dev.
date: 2020-12-17 14:33:08.230000000 +00:00
description: iOSアプリのバージョン番号のルールや判別方法に悩む開発者向けに、分かりやすい比較手法と解決策を紹介し、効率的なバージョン管理を実現します。
image:
  path: "/assets/c4d7c2ce5a8d/1*73CuWIMwmWT1ZsJB8K_q5g.jpeg"
last_modified_at: 2024-04-13 08:39:36.458000000 +00:00
...
- japanese
- ai-translation
title: iOS APP バージョン番号の基本ルール｜判別と比較の最適解を解説
---
### iOS APP バージョン番号について
...
まず「[セマンティックバージョニング](https://semver.org/lang/zh-TW/){:target="_blank"}」という規格を紹介します。これはソフトウェアの依存関係や管理の問題を解決するためのもので、私たちがよく使うCocoapodsなどに適用されます。例えば、Moya 4.0を使っているとします。Moya 4.0はAlamofire 2.0.0に依存しています。もしAlamofireが更新された場合、それが新機能の追加か、バグ修正か、あるいは互換性のない大幅な構造変更かもしれません。このとき、バージョン番号に共通のルールがなければ、どのバージョンが互換性があり更新可能か分からず、混乱が生じます。
...
また、修正番号の後に「プレリリース情報（例: 1.0.1-alpha）」や「ビルドメタデータ（例: 1.0.0-alpha+001）」を付けることもできますが、iOSアプリのバージョン番号としてはこれらの形式はApp Storeにアップロードできないため、ここでは詳しく説明しません。詳細は「[語意化版本](https://semver.org/lang/zh-TW/){:target="_blank"}」を参照してください。
...
実際には 1 == 1.0.0 であることは分かっていますが、この方法で判定すると `.orderedDescending` となります；[こちらの記事で0を補ってから判定する方法](https://sarunw.com/posts/how-to-compare-two-app-version-strings-in-swift/){:target="_blank"} を参考にしてください。通常はAPPのバージョン形式を決めたら変えずに、x.y.z ならずっと x.y.z を使い、x.y と途中で変えないようにしましょう。

#### 複雑な方法

既存のライブラリを直接使用可能： [mrackwitz/Version](https://github.com/mrackwitz/Version){:target="_blank"} 以下は自作の実装です。
...
1. **まずは Version オブジェクトを定義する必要があります：**
...
Regex は「 [語意化バージョン文書](https://semver.org/lang/zh-TW/#%E6%9C%89%E5%BB%BA%E8%AD%B0%E7%94%A8%E6%96%BC%E6%AA%A2%E6%9F%A5%E8%AA%9E%E6%84%8F%E5%8C%96%E7%89%88%E6%9C%AC%E7%9A%84%E6%AD%A3%E8%A6%8F%E8%A1%A8%E7%A4%BA%E5%BC%8Fregex%E5%97%8E){:target="_blank"} 」で提供されている Regex を参考に修正したものです：
...
ただ、現在はすでに非常に完成度の高い [Version](https://github.com/mrackwitz/Version){:target="_blank"} が Pod プロジェクトで利用できるため、わざわざ新しく作る必要はなく、単にビルドプロセスを整理したいだけですXD。
...
- [Swiftで2つのアプリのバージョン文字列を比較する方法](https://sarunw.com/posts/how-to-compare-two-app-version-strings-in-swift/){:target="_blank"}
...
*[Post](https://medium.com/zrealm-ios-dev/ios-app-%E7%89%88%E6%9C%AC%E8%99%9F%E9%82%A3%E4%BA%9B%E4%BA%8B-c4d7c2ce5a8d){:target="_blank"} は [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"} によって Medium から変換されました。*
...
description: Github Action WorkflowをGoogle Apps Script Web Appフォームと連携し、JiraやAsana、Slackなどのツールと統合。開発効率を最大35%向上させる具体的手法を解説します。
image:
...
- japanese
- ai-translation
title: Google Apps Script Web AppでGithub Action CI/CDを効率化｜フォーム連携で開発スピード向上
---
...
![左：元の Github Action Workflow フォーム / 右： [最終成果 (GAS Web App フォーム)](https://script.google.com/macros/s/AKfycbw8SuK7lLLMdY86y3jxMJyzXqa5tdxJryRnteOnNi-lK--j6CmKYXj7UuU58DiS0NSVvA/exec){:target="_blank"}](/assets/4cb4437818f2/1*TiGXBQdPaCM6r2J1RHrgnA.png)

左：元の Github Action Workflow フォーム / 右： [最終成果 (GAS Web App フォーム)](https://script.google.com/macros/s/AKfycbw8SuK7lLLMdY86y3jxMJyzXqa5tdxJryRnteOnNi-lK--j6CmKYXj7UuU58DiS0NSVvA/exec){:target="_blank"}
...
Function as a Service には多くの種類がありますが、[Cloud Functions](https://cloud.google.com/functions?hl=zh-tw){:target="_blank"} は自由度が高いです。しかし、組織のIT制限により、自由にパブリッククラウド関数を追加できず、料金の問題もあるため、やはり私たちの古くからの友人である Google Apps Script に戻ります。

> *以前にGoogle Apps Scriptを使った自動化に関する記事をいくつか書きました。興味のある方はご参照ください：*

> *1. **「 [Google Apps Scriptを使った日次データレポートのRPA自動化](../f6713ba3fee3/) 」***

> *2. 「 [簡単3ステップ — 無料で作るGA4自動データ通知ロボット](https://medium.com/zrealm-robotic-process-automation/%E7%B0%A1%E5%96%AE-3-%E6%AD%A5%E9%A9%9F-%E6%89%93%E9%80%A0%E5%85%8D%E8%B2%BB-ga4-%E8%87%AA%E5%8B%95%E6%95%B8%E6%93%9A%E9%80%9A%E7%9F%A5%E6%A9%9F%E5%99%A8%E4%BA%BA-1e85b8df2348?source=collection_home---6------1-----------------------){:target="_blank"} 」*

> *3. 「 [Crashlytics + Google Analyticsで自動的にアプリのクラッシュフリーユーザー率を照会](https://medium.com/zrealm-robotic-process-automation/crashlytics-google-analytics-%E8%87%AA%E5%8B%95%E6%9F%A5%E8%A9%A2-app-crash-free-users-rate-793cb8f89b72?source=collection_home---6------8-----------------------){:target="_blank"} 」*

> *4. 「 [Crashlytics + Big Query を使ったよりリアルタイムで便利なクラッシュ追跡ツールの構築](https://medium.com/zrealm-robotic-process-automation/crashlytics-big-query-%E6%89%93%E9%80%A0%E6%9B%B4%E5%8D%B3%E6%99%82%E4%BE%BF%E5%88%A9%E7%9A%84-crash-%E8%BF%BD%E8%B9%A4%E5%B7%A5%E5%85%B7-e77b80cc6f89?source=collection_home---6------9-----------------------){:target="_blank"} 」*

総じて、Google Apps Script は Google のもう一つの Function as a Service サービスで、主な特徴は無料で Google サービスと素早く連携できることです。しかし制限も多く、例えば専用の言語しか使えない、実行時間が6分を超えられない、実行回数の上限がある、マルチスレッドに対応していないなどがあります。詳細は[私の以前の記事](../f6713ba3fee3/)をご参照ください。
...
- **Google Apps ScriptのdoGet、doPostメソッドではHeadersを取得できません。**  
  **これにより、[公式のセキュリティ認証](https://api.slack.com/authentication/verifying-requests-from-slack){:target="_blank"}が利用できず、[Slackのリトライ機能](https://api.slack.com/authentication/verifying-requests-from-slack){:target="_blank"}を無効にすることもできません。**
...
> *「アクセス可能なユーザー：XXX 同じ組織内のすべてのユーザー」＋「実行者：ウェブアプリにアクセスするユーザー」を選択することで、**組織アカウントの人だけが使用できるように自動で制限**され、かつ自分の権限で実行されます！*
...
> *もし「XXX」へのアクセス権がブロックされ、Google認証手続きが完了していない場合は、[私の最新記事のGCP設定を参照してください。](../4273e57e7148/)*
...
![[Demo Web App フォーム](https://script.google.com/macros/s/AKfycbw8SuK7lLLMdY86y3jxMJyzXqa5tdxJryRnteOnNi-lK--j6CmKYXj7UuU58DiS0NSVvA/exec){:target="_blank"}](/assets/4cb4437818f2/1*9aaNeemezNPRlSgbLFrseA.png)
...
Slack API の使用については [公式ドキュメント](https://api.slack.com/methods/chat.postMessage){:target="_blank"} を参照してください。
...
*[Post](https://dev.zhgchg.li/%E4%BD%BF%E7%94%A8-google-apps-script-web-app-%E8%A1%A8%E5%96%AE%E4%B8%B2%E6%8E%A5-github-action-ci-cd-%E5%B7%A5%E4%BD%9C-4cb4437818f2){:target="_blank"} Mediumから変換されたもの by [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"}.*
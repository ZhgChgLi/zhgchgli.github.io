...
description: HTML解析に悩む開発者向けに、ZMarkupParserを使ったNSAttributedString変換の手法を解説。手作りレンダリングエンジンで高速かつ正確な描画を実現し、開発効率と表示品質を大幅に向上させます。
image:
...
- japanese
- ai-translation
title: HTML解析器開発：ZMarkupParserでNSAttributedStringへの高速変換｜手作りレンダリングエンジンの実践
---
...
去年発表した記事「[ **TL;DR]** [自行実装 iOS NSAttributedString HTML Render](../a8c2d26cc734/)」では、XMLParserを使ってHTMLを解析し、NSAttributedString.Keyに変換する方法を簡単に紹介しました。記事内のコード構成や考え方は非常に散漫で、当時は問題点の記録として残しただけで、このテーマにあまり時間を割いていませんでした。
...
例：`<b>Test<a>Link</a></b>` は **Test [Link](https://blog.zhgchg.li){:target="_blank"}** のように表示できる必要があります。
...
- [iOS ≥ 12 から断続的にクラッシュする問題で公式の解決策はなし](https://developer.apple.com/forums/thread/115405){:target="_blank"}

- iOS 15 で [大量クラッシュ](../a8c2d26cc734/) が発生し、テストの結果、低バッテリー状態では 100% クラッシュすることが判明しました（iOS ≥ 15.2 で修正済み）
...
第二の方法は[前回の記事](../a8c2d26cc734/)で紹介したもので、XMLParserを使って対応するNSAttributedStringのキーに解析し、スタイルを適用する方法です。
...
- 容錯率 0： `<br>` / `<Congratulation!>` / `<b>Bold<i>Bold+Italic</b>Italic</i>`
  上記の3種類のHTMLの場合、XMLParserで解析すると必ずエラーが発生し、空白が表示されます。
...
- [johnxnguyen](https://github.com/johnxnguyen){:target="_blank"} / [Down](https://github.com/johnxnguyen/Down){:target="_blank"}  
  Markdown を入力として Any (XML/NSAttributedString など) に変換することのみサポートしており、HTML を入力としての変換はサポートしていません。
...
- [malcommac](https://github.com/malcommac){:target="_blank"} / [SwiftRichString](https://github.com/malcommac/SwiftRichString){:target="_blank"}  
  内部では XMLParser を使用しており、前述のケースでも同様に容錯率 0 の問題が発生します。

- [scinfu](https://github.com/scinfu){:target="_blank"} / [SwiftSoup](https://github.com/scinfu/SwiftSoup){:target="_blank"}  
  HTMLパーサー（セレクター）のみ対応し、[NSAttributedStringへの変換は未対応](https://github.com/scinfu/SwiftSoup/issues/127){:target="_blank"}。
...
> 詳細な紹介とインストール・使用方法は、こちらの記事をご参照ください：「[**ZMarkupParser HTML String から NSAttributedString 変換ツール**](../a5643de271e4/)」

直接 [git clone プロジェクト](https://github.com/ZhgChgLi/ZMarkupParser){:target="_blank"} して、`ZMarkupParser.xcworkspace` を開き、`ZMarkupParser-Demo` ターゲットを選択してそのままビルド＆実行して試せます。
...
![運作流程総覧](/assets/2724f02f6e7/1*YF5L7gefMCMwU1wmnGgy6A.png)
...
![運作フローは上図の通りです](/assets/2724f02f6e7/1*U50CX56M_xy1EXZKb69YeA.png)
...
> 元のコードの [HTMLParsedResultFormatterProcessor.swift](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/HTML/Processor/HTMLParsedResultFormatterProcessor.swift){:target="_blank"} の実装対応
...
![如上図](/assets/2724f02f6e7/1*40z0o7R0OROURWCQVDmKrw.png)
...
また [Visitor Pattern](../ba5773a7bfea/) を組み合わせて使用し、各スタイル属性を Element オブジェクトとして定義し、異なる Visit 戦略でそれぞれの適用結果を取得します。
...
> 対応するソースコードの [HTMLTagNameVisitor](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/HTML/HTMLTag/HTMLTagNameVisitor.swift){:target="_blank"} 実装

> また、W3C wiki を参照し、HTML タグ名の列挙型を示しています: [WC3HTMLTagName.swift](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/HTML/HTMLTag/WC3HTMLTagName.swift){:target="_blank"}
...
> 元のコード中の [HTMLTagNameToHTMLMarkupVisitor](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/HTML/Processor/HTMLTagNameToHTMLMarkupVisitor.swift){:target="_blank"} の実装対応
...
![運作結果如上図](/assets/2724f02f6e7/1*D-oMszCDzsBpUYnCEWGKHQ.png)
...
> 対応するソースコードの [HTMLSelector](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/HTML/Processor/HTMLSelector.swift){:target="_blank"} 実装
...
> 元のコード内の [MarkupStyle](https://github.com/ZhgChgLi/ZMarkupParser/tree/main/Sources/ZMarkupParser/Core/MarkupStyle){:target="_blank"} 実装対応

> また、W3c wiki やブラウザの事前定義カラー名も参照し、対応するカラー名テキストとカラーの R,G,B 列挙型を列挙しています: [MarkupStyleColorName.swift](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/Core/MarkupStyle/MarkupStyleColorName.swift){:target="_blank"}
...
私たちは一部のデフォルトスタイルを [MarkupStyle](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/Core/MarkupStyle/MarkupStyle%2BExtension.swift){:target="_blank"} に定義しており、外部コードからタグのスタイルが指定されていない場合、一部のマークアップはこのデフォルトスタイルを使用します。
...
> 対応するソースコード [HTMLElementWithMarkupToMarkupStyleProcessor.swift](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/HTML/Processor/HTMLElementWithMarkupToMarkupStyleProcessor.swift){:target="_blank"} の実装
...
> 対応するソースコード [MarkupNSAttributedStringVisitor.swift](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/Core/Processor/MarkupNSAttributedStringVisitor.swift){:target="_blank"} の実装

![運作フローと結果は上図の通り](/assets/2724f02f6e7/1*gJA_6uM5tQw2kUJsqIssuw.png)
...
> 対応するソースコード [ZHTMLParserBuilder.swift](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/HTML/ZHTMLParserBuilder.swift){:target="_blank"} の実装
...
Beter 部分は [設定 headIndent, firstLineHeadIndent, NSTextTab](https://www.objc.io/issues/9-strings/string-rendering/){:target="_blank"} を使って実現できる可能性がありますが、テストしたところ文字列が長すぎたりサイズが変わったりすると、完璧に表示できませんでした。
...
**リスト記号のサポート一覧はこちらをご参照ください：** [MarkupStyleList.swift](https://github.com/ZhgChgLi/ZMarkupParser/blob/main/Sources/ZMarkupParser/Core/MarkupStyle/MarkupStyleList.swift){:target="_blank"}
...
NSAttributedString で画像を表示するのは想像以上に複雑で、うまく実装できる方法がありません。以前 [UITextView 文繞圖](../e37d66ea1146/) を作ったときに少し苦労しましたが、今回改めて調べてみても完璧な解決策は見つかりませんでした。
...
主に [Asynchronous NSTextAttachments](https://www.cocoanetics.com/2016/09/asynchronous-nstextattachment-1/){:target="_blank"} のシリーズ記事を参考に実装しましたが、最後の更新内容部分（ダウンロード完了後にUIをリフレッシュして表示する）を置き換え、さらに外部拡張用にDelegate/DataSourceを追加しています。

![運用の流れと関係は上図の通り](/assets/2724f02f6e7/1*JZ8IVVNj9B2l-UBemGbAig.png)
...
`NSLayoutManager.invalidateLayout(forCharacterRange: range, actualCharacterRange: nil)` や `NSLayoutManager.invalidateDisplay(forCharacterRange: range)` を使って UI を更新しない理由は、UI が正しく表示更新されないことが判明したためです。範囲が分かっているので、直接 NSAttributedString を置き換えることで UI の正しい更新を保証できます。
...
主な機能ロジックにはすべて UnitTests があり、統合テストも加えています。最終的な [Test Coverage](https://app.codecov.io/gh/ZhgChgLi/ZMarkupParser){:target="_blank"} は **85%** 前後です。
...
[Codecov.io](https://about.codecov.io){:target="_blank"}（パブリックリポジトリは無料）と連携してテストカバレッジを評価するには、CodecovのGitHubアプリをインストールして設定するだけです。
...
- `?:` は ( ) 内のマッチ結果をグループ化しますが、キャプチャはしません。  
  例：`(?:https?:\/\/)?(?:www\.)?example\.com` は `https://www.example.com` に対して、`https://` や `www` ではなく、全体のURLを返します。

- `.+?` は非貪欲マッチ（最も近いものを見つけたら返す）です。  
  例: `<.+?>` は `<a>test</a>` の中で `<a>` と `</a>` を返し、文字列全体ではありません。

- `(?=XYZ)` は `XYZ` という文字列が現れるまでの任意の文字列を意味します。注意すべきは、似ている `[^XYZ]` は `X` または `Y` または `Z` のいずれかの文字が現れるまでの任意の文字を表すことです。  
  例：`(?:__)(.+?(?=__))(?:__)` （`__` までの任意の文字列）は `test` にマッチします。

- `?R` は再帰的に同じルールの値を内部で検索します。  
  例：`\((?:[^()]\\|((?R)))+\)` は `(simple) (and(nested))` に対して `(simple)` 、 `(and(nested))` 、 `(nested)` をマッチします。
...
- [正規表現はどのように動作するのか？](https://mp.weixin.qq.com/s/i_C4ATnajxRDGlTA8dJDHg){:target="_blank"} -> **今後このプロジェクトの正規表現のパフォーマンスを改善する際に参考にできます**

- [Regex エラーによる無限検索が原因でサーバー障害が発生した事例](https://juejin.cn/post/6850418120390082574){:target="_blank"}

- [Regex101 右下にすべての正規表現ルールを確認できます](https://regex101.com){:target="_blank"}
...
3. [ZNSTextAttachment](https://github.com/ZhgChgLi/ZNSTextAttachment){:target="_blank"} をさらに最適化し、再利用可能にしてメモリを解放する；CoreTextの研究が必要かもしれません
...
8. たくさんのご意見お待ちしております。[issue](https://github.com/ZhgChgLi/ZMarkupParser/issues){:target="_blank"}を開いてください。

> [もしご支援いただけるなら、⭐を押していただくことでリポジトリがより多くの人に見られ、GitHubの達人たちが協力してくれる可能性が高まります！](https://github.com/ZhgChgLi/ZMarkupParser){:target="_blank"}
...
現在、弊社の [pinkoi.com](https://www.pinkoi.com){:target="_blank"} の iOS アプリで使用しており、問題は発生していません。😄
...
*[Post](https://medium.com/zrealm-ios-dev/%E6%89%8B%E5%B7%A5%E6%89%93%E9%80%A0-html-%E8%A7%A3%E6%9E%90%E5%99%A8%E7%9A%84%E9%82%A3%E4%BA%9B%E4%BA%8B-2724f02f6e7){:target="_blank"} Mediumから[ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"}で変換。*
...
5年が経ち、「非連続データを処理しない」という妥協点に対して、よりクリーンな解決策が見つかりました — 「どのバイト範囲がキャッシュされているか」という**集合問題**を抽出し、ジェネリックで整数座標の閉区間コンテナに任せる方法です：[Rangeable](https://github.com/ZhgChgLi/RangeableRFC){:target="_blank"} 。

> *Rangeable は `Hashable` 要素を「結合された非重複整数区間集合」にマッピングするコンテナで、もともとは ZMediumToMarkdown の markdown レンダラーから抽出されたものです。同じ API が本記事の非連続キャッシュ問題も解決できるため、このケースを [RFC §1.3.1](https://github.com/ZhgChgLi/RangeableRFC/blob/main/RFC.md){:target="_blank"} に第2のリファレンスコンシューマーとして追加しました。*

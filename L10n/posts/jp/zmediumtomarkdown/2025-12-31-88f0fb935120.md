...
description: MediumのGraphQL Private APIを活用し、記事のデータ収集とバックアップを効率化。Cloudflare認証のブロックを回避する具体的手法を解説し、安定したスクレイピング環境を実現します。
image:
...
Medium Graphql Private API を使ったデータ取得、記事バックアップおよび Cloudflare 認証回避方法。

#### 背景

2018年からMediumで記事を書き始め、2022年頃には記事数が50本以上、文字数は6万字以上、記事画像は約3GBに達しました（2025年までに120本以上、10万字以上、6GB以上の画像に増加）。**バックアップなし！バックアップなし！バックアップなし！** ずっとMediumのエディターで直接記事を書いており（エディターは本当に使いやすい）、**書き終えたらすぐに保存・公開し、ローカルには一切バックアップを取っていません。**

あの時、不安な気持ちが自然と湧いてきました。もしある日 Medium がなくなったら、私の記事も消えてしまうのではないかと。（しかもその数年間、Medium の状況はずっと良くなく、収益も出ていませんでした）あるいは、突然アカウントが停止されたら、今までの努力が全部無駄になってしまうのではないかと。

そこでその年から Medium のデータ、特に記事内容の取得とダウンロード・バックアップ方法の研究を始めました。

#### Medium HTML ❌

構造が非常に複雑で変わりやすく、クロールが遅いため、ブラウザを直接使ってコンテンツを取得するのは最悪の方法です。
...
> ***警告 MediumのAPIはすでにサポートされていません。使用は推奨されません。***

Medium はかつて公式の API を提供していましたが、おそらくリリース後まもなく停止されました。製品の目的が異なるためと推測されます。Medium はプライベートなトラフィックを重視し、コンテンツをサイト内に留めることを優先しており、外部が API を通じてデータを取得できるようにはしていません。

#### Medium 非公式API 🤔

[![](https://mediumapi.com/imgs/mediumapi-website-image.jpg)](https://mediumapi.com/){:target="_blank"}

また、公式が API を提供していないため、有志の開発者が外部向けの API を独自に開発し、必要なユーザーが接続できるようにしています。しかし、その仕組みはおそらく Medium の Private API を利用しており、その一層のラッピングやブリッジを通して、リクエストを Private API に変換し、元のデータを取得していると推測しています。

> ***料金も安くなく、約2,500リクエストで5米ドル*** *；このサービスはかなり長く続いているが、結局はMediumのプライベートAPIを叩いているので、自分で解析して自分で叩けばいい。*

### Medium Private API ✅

何が Private API か？それはこの API インターフェースが **外部向けではなく、ドキュメントがなく、いつでも停止される可能性があり、法的リスクがある** ということです。つまり、**直接そのサイトやアプリの動作を見て呼び出されている API を見つけ出し、自分でプログラムを書いてその API を使い、自分のサービスにデータを取得する** ということです。

#### <https://medium.com/_/graphql>{:target="_blank"}

ネットワークリクエストの解析を通じて、すべての Medium API は <https://medium.com/_/graphql>{:target="_blank"} というエンドポイントを使用していることがわかります。自分のプログラムで同じデータを呼び出して取得できれば、この方法は実現可能です。
...
したがって、私は Medium 上の自分のアカウントのフォロワー数をスクレイピングする必要があります。

- ブラウザの「開発者モード」を開きます（Chrome → 右クリック → 検証）。「Network」タブに切り替え、「Preserve log」をチェックし、検索欄に「`graphql`」と入力します。

- Medium アカウントのフォロワーページにアクセス：「 `https://medium.com/@YOUR_USER_NAME/followers` 」
  フォロワーのAPIを取得できるのはこの形式のURLのみで、`https://YOUR_USER_NAME.medium.com/followers` の形式では呼び出されません。

![](/assets/88f0fb935120/1*jmGbo-BQK9amNmoRneJRPw.png)

- 左側の「graphql」リクエストを一つずつ確認し、「Response」データ内で「`followerCount`」というキーワードを検索します。このキーワードが含まれ、かつ`username`がページのユーザー名と一致し、フォロワー数がページ表示と同じであれば、その対象リクエストを見つけます。

- ターゲットの API を見つけたら、Payload → View Source → 以下の全内容をコピーしてください。

![](/assets/88f0fb935120/1*CGyScjCEe9Cf_3VIJrUJzQ.png)

現在私たちは Medium のフォロワー取得用 API とペイロードを手に入れたので、[Postman](https://www.postman.com/){:target="_blank"} で試してみましょう！
...
Authorization: 認証不要（トークンやクッキーの設定は不要）
...
def load_medium_followers(username)
    begin
        ...
        # CRL関連のフラグをクリアして厳しい失敗を回避しつつ、ピア証明書を検証します。
        https.verify_mode = OpenSSL::SSL::VERIFY_PEER

        store = OpenSSL::X509::Store.new
        store.set_default_paths
        # デフォルトでCRLチェックフラグが有効になっていないことを保証
        store.flags = 0
...
- `id` : アカウントに対応するユーザーIDで、上記のフォロワー取得エンドポイントで提供されます

- `homepagePostsFrom` : ページネーションのパラメータで、レスポンス内の `from` を使ってすべてのページデータを取得できます
...
- `posdId` : 記事のIDで、上記で取得した記事一覧のAPIから取得できます。
...
- `response[0]["data"]["post"]["viewerEdge"]["fullContent"]["bodyModel"]["paragraphs"]` ：記事全体の段落で、これらを組み合わせると全文になります。
...
- 自分の有料記事をスクレイピングする場合は、sid、uid のログイントークン情報を付与する必要があります（以下を参照）。

### Medium x Cloudflare 攻防

本記事の第二のテーマはCloudflareです。2025年下半期頃から、MediumのCloudflare防御設定はほぼ最高レベルになり、クラウドサービスからのすべてのリクエストがブロックされるようになりました（私が使っているGoogle Apps ScriptやGitHub Actionsもすべてブロックされました）、そのためデータのスクレイピングに失敗しました。
...
これには非常に困っています。なぜなら、私のミラーサイト ( <https://zhgchg.li>{:target="_blank"} ) はすでに数年間安定稼働しており、Medium に新しい記事が公開されると、そこで定期的にスクリプトが自動実行されて同期されるからです（Private API Graphql を通じて）。**もしクラウドサービスがブロックされると、手動でローカルのパソコンでスクリプトを実行するしかありません**。

#### **Header Cookies に sid, uid を追加 ❌**

**最初は Header の Cookies に sid や uid を追加することで Medium のログイン状態となり、問題なく通過できましたが、数ヶ月後にはこの方法も使えなくなりました。**
...
#### Cloudflare Worker ブリッジリクエスト ✅

いろいろ試した後に方法が見つからず、ひらめいて Cloudflare の魔法で魔法を打ち破りました。Cloudflare を使ったら、なんと成功しました！Cloudflare のボット防護にブロックされませんでした。
...
以上が私がMedium APIと長年戦ってきた軌跡です。現在、すべての記事（Markdownファイル）と画像は <https://zhgchg.li>{:target="_blank"} にバックアップされています。すべての添付画像ファイルも含まれており、GitHubリポジトリとローカルのハードディスクにも保存しているので非常に安全です。

> やはり Medium が長く続くことを願っています！この記事の内容は実験的な参考用であり、著者は一切の使用責任を負いません。

ご質問やご意見がありましたら、[こちらからご連絡ください](https://www.zhgchg.li/contact){:target="_blank"} 。

*[Post](https://zhgchgli.medium.com/medium-private-api-%E8%B3%87%E6%96%99%E7%88%AC%E8%9F%B2%E8%88%87-cloudflare-%E6%94%BB%E9%98%B2%E7%9A%84%E5%BF%83%E8%B7%AF%E6%AD%B7%E7%A8%8B-88f0fb935120){:target="_blank"} Mediumから変換したもの [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"}.*
...
> *先用 AI 重構了幾個開源專案的應用層面，因而得到啟發，也讓他做深入的 Foundation 資料結構設計。*
...
最後還請 AI 建立 Medium.com 文章 Reader MCP 服務。
...
> [*mcp-medium-reader*](https://github.com/ZhgChgLi/mcp-medium-reader){:target="_blank"} _能讓你的 AI 穿透封鎖並且讀取 Markdown 版本的 Medium 文章，節省 Token 的同時又能提升 AI 理解性。*

### **帶標籤區間集合問題**



> *當年在做 [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"} 的時候有遇到 **這個 Foundation 資料結構設計(算法)問題**，那時候沒有多餘的精力也沒有能力解決，但現在有 AI 了，想想可以嘗試用 AI 解決這個問題。*
...
> *當時也是搞了很久想了老半天，書到用時方恨少，平時刷題太少，遇到實際要用的時候沒武器可用；那時候是用最暴力的 walkthrough 解決，結果正確只是效能跟程式碼很可怕。*
...
當時一樣遇到區間計算問題 — **Covered / Uncovered Interval Query**，不過比 Medium 問題簡單，因為這邊是 Binary Data 0 跟 1 的區別而已，只需要計算結果。

> *當時也是暫時沒解，因為光開發 AVPlayer 邊播放邊緩存的功能就花大半時間了；加上當時場景是音訊，檔案本身就不大； **直接先走區間沒資料就全拿覆蓋的方式 — 細節請參考原文章「 [AVPlayer 本地 Cache 實作攻略｜使用 AVAssetResourceLoaderDelegate 節省 iOS 音樂串流流量](../6ce488898003/) 」** 。*
...
[![](https://opengraph.githubassets.com/d13a720075318557bd1440f067642e7552a7446ff9748eeff5d770749ada3342/ZhgChgLi/RangeableRFC)](https://github.com/ZhgChgLi/RangeableRFC/blob/main/RFC.md){:target="_blank"}
...
> 以下 RFC 細節除非很有興趣，不然可以直接略過。這邊也是直接用 AI 翻譯總結的。
...
RFC 明確指出共通問題是：給定很多 `(eᵢ, lᵢ, hᵢ)` ，查某個位置 `i` 時，要回傳所有滿足 `lᵢ ≤ i ≤ hᵢ` 的元素。
...
> *最困難的 RFC 規格研究已經完成，後面的實作只要 AI 遵照這個規格實現，幾乎都不會有問題。*
...
效果超乎我的想像，大略閱讀了他寫的 RFC，不管是格式還是內容都不比真實的人去研究寫出來的東西差（至少一定比我寫的好），中間有觀察他在做什麼，他會用 Web Search 去搜尋公開的相關論文或是技術文獻然後整合評估實作合理性；拆分出 doer / reviewer 的效果也很顯著，doer 會太著重在實作而 reviewer 能以更廣更高的視角來縱觀看 doer 做的東西有沒有 side effect；最後 RFC 定下後再請 AI 基於這個文件實作，準確度幾乎是 100%。

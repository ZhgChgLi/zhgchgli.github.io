...
description: Discover practical alternatives to iOS NSAttributedString's DocumentType.html
  rendering. This guide helps developers overcome HTML display limitations, enabling
  smoother text rendering and enhanced app performance.
image:
...
- english
- ai-translation
title: 'iOS NSAttributedString HTML Render: Alternative Methods for DocumentType.html'
---
...
After some searching, I found many similar cases online and also found the earliest identical [crash issue thread](https://developer.apple.com/forums/thread/115405){:target="_blank"} on the App Developer Forums, which received a response from the **official** team:
...
- Directly Using Markdown as Rendering Constraint: iOS ≥ 15 NSAttributedString can [directly render text using Markdown format](https://developer.apple.com/documentation/foundation/nsattributedstring/3796598-init){:target="_blank"}
...
> The pitfalls of using UITextView for text wrapping can be referenced in my previous article: [iOS UITextView 文繞圖編輯器 (Swift)](../e37d66ea1146/)
...
> Even if you are using iOS < 15 which does not support native Markdown, you can still find [expert-made Markdown Parser solutions](https://github.com/chockenberry/MarkdownAttributedString){:target="_blank"} on Github.

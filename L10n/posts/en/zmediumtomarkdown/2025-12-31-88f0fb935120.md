...
description: Discover how to scrape Medium data using GraphQL Private API, overcome
  Cloudflare verification blocks, and securely back up articles. Perfect for developers
  facing API access challenges seeking reliable solutions.
image:
  path: /assets/88f0fb935120/1*Lh9a_XR4zvvsaP3y58x9Aw.png
last_modified_at: 2025-12-31T04:36:53.706+0000
...
### Medium Private API Data Crawling and the Battle with Cloudflare: A Personal Journey

Using Medium Graphql Private API to Crawl Data, Backup Articles, and Bypass Cloudflare Verification Blocking Methods

#### Background

Since I started writing on Medium in 2018, by around 2022 I had accumulated over 50 articles, nearly 60,000 words plus about 3 GB of article images (by 2025, over 120 articles / 100,000+ words / 6 GB+ images); **no backups! no backups! no backups!** Because I always wrote directly on Medium (the editor is really convenient), **I published immediately after writing with no local backups at all.**

At that time, a sense of unease arose—what if one day Medium shuts down and my articles disappear? (And Medium had been struggling for years, not making money.) Or if my account suddenly gets banned, wouldn't all my hard work be lost?

So that year, I began researching how to scrape Medium data, especially article content and how to download backups.

#### Medium HTML ❌

The structure is very complex, prone to changes, and slow to crawl. Directly scraping content via the browser is the worst approach.
...
Medium once provided an official API, but it was likely discontinued shortly after launch; the probable reason is a different product goal. Medium focuses on private traffic and prefers to keep content within the site rather than allowing external access through an API.

#### Medium Unofficial API 🤔

[![](https://mediumapi.com/imgs/mediumapi-website-image.jpg)](https://mediumapi.com/){:target="_blank"}

Since the official API is not provided, some community experts have developed external API interfaces for users in need; however, I suspect the underlying principle is still calling the Medium Private API, just with an additional layer of encapsulation that bridges and converts requests to the Private API to retrieve the original data.

> ***The fee is not cheap, about $USD 5 for every 2,500 requests*** *; this service has been running for quite a while, but since it ultimately calls the Medium Private API, I might as well sniff and call it myself.*

### Medium Private API ✅

What is a Private API? It is an API endpoint **not intended for external use, undocumented, possibly subject to sudden shutdown, and may carry legal risks**; basically, **directly observing the website/App behavior to find the corresponding API calls**, then writing your own code to reuse this API for fetching data into your own service.

#### <https://medium.com/_/graphql>{:target="_blank"}

By sniffing network requests, we can see that all Medium APIs use the interface <https://medium.com/_/graphql>{:target="_blank"}. As long as we can call this in our own program and get the same data, this approach can succeed.

#### Medium Private API — Sniffing Medium Graphql (Example: Getting Follower Count)

My first application was to crawl my follower count and display it on my portal website.

[**link.zhgchg.li**](https://link.zhgchg.li/){:target="_blank"} **:**

![](/assets/88f0fb935120/1*Lh9a_XR4zvvsaP3y58x9Aw.png)

Therefore, I need to crawl the follower count of my account on Medium.
...
- Go to the Medium account followers page: `https://medium.com/@YOUR_USER_NAME/followers`  
  Only the follower URL in this format can be sniffed; the format `https://YOUR_USER_NAME.medium.com/followers` does not call the Followers API.

![](/assets/88f0fb935120/1*jmGbo-BQK9amNmoRneJRPw.png)

- Check each "graphql" request on the left, search for the keyword `followerCount` in the "Response" data, and find the request where the `username` matches the webpage's username and the follower count matches the number shown on the page. This is the target request.

- After finding the target API, switch to Payload -> View Source -> copy all the content below

![](/assets/88f0fb935120/1*CGyScjCEe9Cf_3VIJrUJzQ.png)

Now we have obtained the Medium followers API & Payload, let's test it using [Postman](https://www.postman.com/){:target="_blank"}!

#### Medium Followers Count Endpoint — Request & Response
...
Key info: `response[0]["data"]["userResult"]["socialStats"]["followerCount"]` is the target follower count!
...
In the same way, we can also get the target account's list of articles.
...
Next is the most crucial part: crawling the original content of the articles.
...
- `response[0]["data"]["post"]["viewerEdge"]["fullContent"]["bodyModel"]["paragraphs"]` : The paragraphs of the entire article; combined, they form the full article content

- `["markups"]` : Text styles in the paragraph, such as bold, hyperlinks…

> ***As for how to convert this JSON description format into Markdown, you can directly refer to or use my previously developed open-source tool — [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"}***
...
- If you want to crawl your own paid articles, you need to include the sid and uid login token information (see below).

### Medium x Cloudflare Attack and Defense

The second topic of this article is Cloudflare. Starting around the second half of 2025, Medium's Cloudflare protection reached nearly the highest level, blocking all requests from cloud services (I was blocked using Google Apps Script / GitHub Actions), causing data scraping to fail.
...
This is very frustrating for me because my mirror site ( <https://zhgchg.li>{:target="_blank"} ) has been running steadily for several years. Whenever Medium publishes new articles, the site automatically runs scripts to sync them over (via the Private API Graphql). **If cloud services get blocked, I can only run the scripts manually on my local computer**.

#### **Add sid, uid to Header Cookies ❌**

**At first, I added sid and uid to the header cookies to represent Medium login credentials, which allowed the requests to pass smoothly. However, after a few months, even this method stopped working.**
...
After trying everything without success, I had a sudden idea to use Cloudflare's magic to defeat the magic. Using Cloudflare, it surprisingly worked! It no longer got blocked by Cloudflare Bot protection.
...
This sums up my years-long battle with the Medium API. Currently, all articles (Markdown files) and images are backed up at <https://zhgchg.li>{:target="_blank"}, including all attached image files. There is a copy on the GitHub repo and another on my computer hard drive, ensuring full safety.

> I still hope Medium will last a long time! The content of this article is for experimental reference only; the author assumes no responsibility for its use.

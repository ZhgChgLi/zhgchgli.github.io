...
description: Discover how to scrape data from Medium's GraphQL Private API, bypass
  Cloudflare verification blocks, and securely back up your articles. This guide helps
  developers facing access restrictions achieve reliable data extraction and storage.
image:
...
- english
- ai-translation
title: Medium API Data Scraping｜Overcoming Cloudflare Challenges for Article Backup
---

### The Journey of Crawling Medium API Data and Battling Cloudflare Defense

Using Medium Graphql Private API to Crawl Data, Backup Articles, and Bypass Cloudflare Verification Blocks.

#### Background

Since I started writing on Medium in 2018, by around 2022 I had accumulated over 50 articles, nearly 60,000 words, and about 3 GB of article images (by 2025, over 120 articles/100,000+ words/6 GB+ images); **no backups! no backups! no backups!** Because I always wrote directly on Medium (the editor is really good), **I published immediately after writing without any local backups.**

At that time, a sense of unease arose—what if one day Medium shuts down and my articles disappear? (And Medium had been struggling for years, not making money.) Or what if my account suddenly gets suspended, would all my hard work be lost?
...
Medium once provided an official API, but it was likely discontinued shortly after its launch. It is presumed that this was due to different product goals—Medium aims to keep content within its own platform rather than allowing external access through APIs.

#### Medium Unofficial API 🤔

[![](https://mediumapi.com/imgs/mediumapi-website-image.jpg)](https://mediumapi.com/){:target="_blank"}

Because the official API is not provided, some skilled developers have created external API interfaces for users in need; however, I suspect these also call the Medium Private API, just adding a layer of encapsulation to bridge and convert requests into the Private API to fetch the original data.

> ***The fee is not cheap, about $5 USD per 2,500 requests*** *; this service has been running for quite a while, but since it ultimately calls the Medium Private API, I might as well sniff and call it myself.*

### Medium Private API ✅

What is a Private API? It is an API interface **not intended for external use, undocumented, may be disabled at any time, and could pose legal risks**; essentially, it means **observing the website/App behavior to find the corresponding API calls** and writing your own code to reuse this API to fetch data for your own service.

#### <https://medium.com/_/graphql>{:target="_blank"}

By sniffing network requests, we found that all Medium APIs use the <https://medium.com/_/graphql>{:target="_blank"} endpoint. As long as we can call this endpoint in our own program and get the same data, this approach will work.
...
Therefore, I had to scrape the follower count of my account on Medium.
...
#### Medium Follower Count Endpoint — Request & Response
...
#### Medium Private API — Fetch All Articles of an Account

In the same way, we can also get the target account's article list.
...
- `id` : The User Id corresponding to the account, provided by the follower retrieval Endpoint above
...
The article list information can be obtained at `response[0]["data"]["userResult"]["homepagePostsConnection"]["posts"]`.
...
- `posdId` : Article ID, provided by the API used to get the article list above.
...
Article source as shown in the image:

- `response[0]["data"]["post"]["viewerEdge"]["fullContent"]["bodyModel"]["paragraphs"]` : The entire article's paragraphs, combined to form the full article content

- `["markups"]` : Text styles in the paragraph, such as bold, hyperlinks…

> ***Regarding how to convert this JSON description format into Markdown, you can directly refer to or use my previously developed open-source tool — [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"}***
...
### Medium x Cloudflare Offense and Defense

The second topic of this article is Cloudflare. Starting around the second half of 2025, Medium’s Cloudflare protection was set to nearly the highest level, blocking all requests from cloud services (my requests from Google Apps Script and GitHub Actions were all blocked), causing data scraping to fail.
...
This was very frustrating for me because my mirror site ( <https://zhgchg.li>{:target="_blank"} ) has been running steadily for several years. Whenever Medium publishes a new article, a script automatically runs there regularly to sync it over (via the Private API Graphql). **If cloud services get blocked, I will have to run the script manually on my local computer**.

#### **Adding sid, uid to Header Cookies ❌**

**At first, I added sid and uid to the Header Cookies, which acted as Medium login credentials and worked smoothly. However, after a few months, even this method stopped working.**
...
Change from `https://medium.com/_/graphql` to your deployed Cloudflare Worker URL.

#### Summary

This sums up my years-long battle with the Medium API. Currently, all articles (Markdown files) and images are backed up at <https://zhgchg.li>{:target="_blank"}, including all attached image files. There is a copy in the GitHub repo and one on my computer’s hard drive, ensuring high security.

> Still hope Medium lasts for a long time! This content is for experimental reference only; the author takes no responsibility for its use.

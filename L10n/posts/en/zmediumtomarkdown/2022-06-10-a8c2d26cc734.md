...
categories:
- ZRealm Dev.
date: 2022-06-10T00:11:59.122+0800
description: Discover effective alternatives to iOS NSAttributedString's DocumentType.html
  rendering. This guide helps developers overcome common HTML rendering limitations,
  enabling better text display and customization in iOS apps for enhanced user experience.
image:
  path: /assets/a8c2d26cc734/1*l93Ay_tGXTRvwS7ofgt5og.jpeg
last_modified_at: 2024-04-14T10:07:40.904+0800
...
- english
- ai-translation
title: iOS NSAttributedString HTML Rendering｜Alternative Methods for DocumentType.html
---
...
Redeveloped using a different approach, the *[**ZMarkupParser HTML String to NSAttributedString Tool**](https://medium.com/zrealm-ios-dev/zmarkupparser-html-string-%E8%BD%89%E6%8F%9B-nsattributedstring-%E5%B7%A5%E5%85%B7-a5643de271e4){:target="_blank"}* is available. For technical details and the development story, please visit *[The Story of Manually Building an HTML Parser](https://medium.com/zrealm-ios-dev/%E6%89%8B%E5%B7%A5%E6%89%93%E9%80%A0-html-%E8%A7%A3%E6%9E%90%E5%99%A8%E7%9A%84%E9%82%A3%E4%BA%9B%E4%BA%8B-2724f02f6e7){:target="_blank"}*.

### Origin

Since the release of iOS 15 last year, the app has been plagued by a persistent crash issue. According to data, in the past 90 days (2022/03/11~2022/06/08), it caused over 2.4K crashes, affecting more than 1.4K users.

![](/assets/a8c2d26cc734/1*r--z0J1P6t5ECfVyb5_OxQ.png)

> According to data, the frequent crash issue seems to have been fixed (or its occurrence reduced) by Apple in iOS ≥ 15.2 versions. The data shows a downward trend.

**Most affected versions:** iOS 15.0.X ~ iOS 15.X.X

It was also found that iOS 12 and iOS 13 had occasional crashes, so this issue has likely existed for a long time. However, the crash rate was nearly 100% in the early versions of iOS 15.
...
When this issue suddenly appeared in large numbers, the development team was completely baffled; re-testing the points in the Crash Log showed no problems, and it was unclear under what conditions users experienced the crash. Until one time, by coincidence, I switched to "Low Power Mode" and then the problem was triggered!! **WTF !!!**

![](/assets/a8c2d26cc734/1*gVfmnCN7QcHO90Y7HyntbA.gif)

### Solution

After some searching, I found many similar cases online and also found the earliest identical [crash issue thread](https://developer.apple.com/forums/thread/115405){:target="_blank"} on the App Developer Forums, with an official response:
...
- Use Markdown Directly as Rendering Constraint: iOS ≥ 15 NSAttributedString can [render text directly using Markdown format](https://developer.apple.com/documentation/foundation/nsattributedstring/3796598-init){:target="_blank"}
...
#### Supplement. Rendering Complex HTML — Want to Create Text with Rich Effects
...
It means the text `code in text, and link in text, and ZhgChgLi, and bold, and I, only i` is:
...
- Characters 50 to 63 should be marked as bold (*Text* format)
- Characters 55 to 69 should be marked as italic (_Text_ format)
```
...
> Pitfalls of UITextView for Text Wrapping can refer to my previous article: [iOS UITextView Text Wrapping Editor (Swift)](https://medium.com/zrealm-ios-dev/ios-uitextview-%E6%96%87%E7%B9%9E%E5%9C%96%E7%B7%A8%E8%BC%AF%E5%99%A8-swift-e37d66ea1146){:target="_blank"}

### Why?

Before implementing the solution, let's first revisit the core issue. Personally, I believe the main cause is not from Apple; the official bug is just the trigger for this problem.

The main issue comes from **treating the App side as Web for rendering**. The advantage is fast Web development, using the same API Endpoint to deliver HTML without distinguishing clients, allowing flexible rendering of any content. The drawbacks are that HTML is not a common interface for Apps, App Engineers cannot be expected to understand HTML, **performance is very poor**, rendering can only run on the Main Thread, results cannot be predicted during development, and supported specifications cannot be confirmed.

Looking further up the chain, the problem mostly stems from unclear original requirements and uncertainty about which specifications the app needs to support. In the rush to move quickly, HTML was directly used as the interface between the app and the web.

#### **Poor Performance**

Additional performance note: testing shows that using `NSAttributedString DocumentType.html` directly is 5 to 20 times slower than implementing custom rendering.

#### Better

Since this is for an app, a better approach is to start from the perspective of app development. Adjusting requirements for an app is much more costly than for the web. Effective app development should be based on iterative adjustments with clear specifications. You need to confirm the supported specifications at the moment, and if changes are needed later, schedule time to expand the specs. Avoid making quick, ad-hoc changes to reduce communication costs and improve work efficiency.

- Confirming the Scope of Requirements

- Confirm Supported Specifications

- Confirm the interface specification (Markdown/BBCode/... can continue to use HTML, but it must be constrained, for example only `<b>/<i>/<a>/<u>`, and the program must **explicitly inform** developers)
...
All three scenarios will cause XMLParser to fail and throw an error, resulting in a blank display.  
When using XMLParser, the HTML string must fully comply with XML rules and cannot tolerate errors like a browser or NSAttributedString.DocumentType.html does for normal display.
...
Rewritten in pure Swift, it uses Regex to parse HTML tags and performs tokenization, analyzes and corrects tag accuracy (fixing missing end tags & misaligned tags), then converts them into an abstract syntax tree. Finally, it uses the Visitor Pattern to map HTML tags to abstract styles, producing the final NSAttributedString result; no parser libraries are used.

— —

### How?

The die is cast. Returning to the main topic, now that we are rendering `NSAttributedString` using HTML, how can we solve the crashes and performance issues mentioned above?
...
Before discussing HTML Render, let's first talk about Strip HTML. As mentioned earlier in the `Why?` section, where the app obtains HTML and which HTML it will receive should be clearly defined in the specifications; it should not be that the app "**might**" receive HTML and therefore needs to strip it.
...
Following the approach of [SwiftRichString](https://github.com/malcommac/SwiftRichString){:target="_blank"}, use **[XMLParser](https://developer.apple.com/documentation/foundation/xmlparser){:target="_blank"}** from Foundation to parse HTML as XML and implement your own HTML Parser & Strip functionality.
...
// I agree to provide personal ID number/passport/residence permit number for cross-border logistics clearance use, and have understood the logistics needs of cross-border goods.
```

Using Foundation XML Parser to process String, implement `XMLParserDelegate` with `currentString` to store the String. Since the String may be split into multiple parts, `foundCharacters` can be called multiple times. In `didStartElement` and `didEndElement`, when the string start and end are found, save the current result and clear `currentString`.

- The advantage is that it also converts HTML entities to actual characters, e.g. `&#103; -> g`

- The advantage is handling complex cases; however, if the HTML is not well-formed, XMLParser will fail, e.g., `<br>` instead of `<br/>`.

> I personally think that for simply stripping HTML, **Option 2 is the better method**. This method is introduced because rendering HTML uses the same principle, so let's start with this simple example :)

### HTML Render w/XMLParser

Using XMLParser for a custom implementation, similar to the Strip approach, we can add corresponding rendering methods based on which Tag is parsed.

Requirements:

- Support Extending Tags to Parse
...
- Style support includes changing font weight, size, underline, line spacing, letter spacing, background color, and text color.

- Does not support complex tags such as Image Tag, Table Tag, etc.

> You can remove features according to your own specifications. For example, if background color adjustment is not needed, you don't have to provide an interface for setting background color.

> This article is just a conceptual implementation, **not a best practice in architecture**; if there are clear specifications and usage methods, consider applying some design patterns to achieve better maintainability and extensibility.

### ⚠️⚠️⚠️ Attention ⚠️⚠️⚠️

A reminder, **if your app is new or can fully switch to Markdown format, it is recommended to use the above method, as implementing your own renderer here is too complex and will not perform better than Markdown**.

> Even if you are using iOS < 15 which does not support native Markdown, you can still find [an excellent Markdown Parser solution](https://github.com/chockenberry/MarkdownAttributedString){:target="_blank"} on GitHub.
...
Implementing Style Attributed Parser to parse `style="color:red;font-size:16px"` but CSS styles have many possible settings, so it's necessary to list the supported range.
...
This completes our implementation of HTML rendering using XMLParser, while maintaining extensibility and specifications. We can manage and understand the supported string rendering types in the app directly from the code.
...
> *This article is also published on my personal blog: [**[Click here]**](https://zhgchg.li/posts/a8c2d26cc734/){:target="_blank"}.*

> *If you have any questions or feedback, feel free to [contact me](https://www.zhgchg.li/contact){:target="_blank"}.*

*[Post](a8c2d26cc734){:target="_blank"} converted from Medium by [ZMediumToMarkdown](https://github.com/ZhgChgLi/ZMediumToMarkdown){:target="_blank"}.*
...
Five years later, looking back, the compromise of "not handling non-continuous data" finally has a cleaner solution — extracting the **set problem** of "which byte ranges have been cached" and delegating it to a generic, integer-coordinate, closed-interval container: [Rangeable](https://github.com/ZhgChgLi/RangeableRFC){:target="_blank"}.

> *Rangeable is a container that maps `Hashable` elements to a merged set of disjoint integer ranges. It was originally extracted from ZMediumToMarkdown's markdown render; the same API happens to solve the non-continuous cache issue discussed in this article, so this case was added to [RFC §1.3.1](https://github.com/ZhgChgLi/RangeableRFC/blob/main/RFC.md){:target="_blank"} as the second reference consumer.*

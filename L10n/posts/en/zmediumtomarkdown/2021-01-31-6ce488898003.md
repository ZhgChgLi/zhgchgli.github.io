...
description: Discover how to implement local caching with AVPlayer and AVQueuePlayer
  using AVURLAsset and AVAssetResourceLoaderDelegate to enhance video playback performance
  and reduce buffering issues effectively.
image:
...
It is also possible to request multiple Range data and set conditions in the same request, but we do not use these features. For details, please [refer to this](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Range_requests){:target="_blank"}.
...
1. The second request mentioned earlier requests the resource from 0 to the total length. Once enough data is received, it will initiate a Cancel to stop the request.
...
For caching, I directly use [PINCache](https://github.com/pinterest/PINCache){:target="_blank"}, delegating the cache management to it. This avoids dealing with cache read/write deadlocks and implementing the LRU cache eviction strategy ourselves.
...
> *⚠️ The `mergeDownloadedDataIfIsContinuted` **method is extremely important.***
...
Five years later, looking back at the compromise of "not handling discontinuous data," there is finally a cleaner solution — extracting the **set problem** of "which byte ranges are cached" and delegating it to a generic, integer-coordinate, closed-interval container: [Rangeable](https://github.com/ZhgChgLi/RangeableRFC){:target="_blank"}.

> *Rangeable is a container that maps `Hashable` elements to a merged set of disjoint integer intervals. It was originally extracted from the markdown render of ZMediumToMarkdown; the same API also solves the non-continuous cache issue in this article, so this case was added to [RFC §1.3.1](https://github.com/ZhgChgLi/RangeableRFC/blob/main/RFC.md){:target="_blank"} as the second reference consumer.*
...
- **(Q2) Writing side:** After receiving a new byte range `[a, b]`, it should automatically merge with existing ranges like `[0, 100]`, `[200, 500]`, etc., including integer-adjacent cases such as `100/101`. The original `mergeDownloadedDataIfIsContinuted` in this article only handled the case where the new data exactly follows the tail; all other cases were discarded.
...
- Write bytes into a sparse file using `FileHandle` (the `FileHandle seek read/write` approach mentioned by [Lex Tang](https://twitter.com/lexrus){:target="_blank"} in the article).

- The byte range index uses `Rangeable<CacheToken>` (replacing all the if-else statements in `mergeDownloadedDataIfIsContinuted`).
...
> *The original `mergeDownloadedDataIfIsContinuted` entire if-else block is removed; instead, `ranges.insert(.cached, ...)` is used, and Rangeable handles overlapping/adjacent merges by itself.*
...
> *No need to change AVPlayer's cancel/seek mechanism: seeking to 5000 triggers a new `loadingRequest`, and the new `cachedPrefix` directly takes data from the 5000 offset (as long as it was downloaded before); canceling the previous request keeps the partially written bytes in the sparse file within the `Rangeable` index, so no data is lost.*
...
Hash can use md5…sha…, on iOS ≥ 13 you can directly use Apple's [CryptoKit](https://developer.apple.com/documentation/cryptokit/){:target="_blank"}, otherwise just search on Github!
